...
def param_grid() -> list[CX32ADSRParams]:
    return [
        CX32ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'escape_border|reverse', 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.02, 0.05, 32, 16, 2, 24),
        CX32ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'reverse_setup|reverse', 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.02, 0.05, 32, 16, 2, 24),
        CX32ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'escape_border|reverse', 0.11, 0.13, 0.31, 0.97, 'forward_safe|forward_turn', 0.02, 0.05, 32, 16, 2, 24),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Dual-Slice-Repair', 'disable_dsr': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX32ADSRParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'dsr_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class DSRPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX32ADSRParams, memory: dict[str, Any], disable_dsr: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_dsr = bool(disable_dsr)
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if not self.disable_dsr and str(self.scene_kind) == 'misc':
...
def make_policy(memory: dict[str, Any], params: CX32ADSRParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return DSRPolicy(case, bundle, field, params, memory, disable_dsr=bool(ablation.get('disable_dsr', False)), enable_diagnostics=bool(ablation.get('enable_diagnostics', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX32ADSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX32ADSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

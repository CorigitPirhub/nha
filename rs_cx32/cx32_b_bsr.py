...
from rs_cx32.common import BaseCX28Policy, CX27WatchdogConfig, build_frozen_haa_stack


@dataclass(frozen=True)
class CX32BBSRParams:
...
def param_grid() -> list[CX32BBSRParams]:
    return [
        CX32BBSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'escape_border|reverse', 1, 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.02, 0.05, 32, 16, 2, 24),
        CX32BBSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'reverse_setup|reverse', 1, 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.02, 0.05, 32, 16, 2, 24),
        CX32BBSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'escape_border|reverse', 2, 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.02, 0.05, 32, 16, 2, 24),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Budgeted-Slice-Repair', 'disable_bsr': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX32BBSRParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'bsr_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class BSRPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX32BBSRParams, memory: dict[str, Any], disable_bsr: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_bsr = bool(disable_bsr)
    ...
    def start_search(self, planner, start, goal, h_pair, search_state):
        ...
        if bool(search_state.get('cx28_active', False)):
            search_state['cx32_rescue_count'] = 0
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if not self.disable_bsr and str(self.scene_kind) == 'misc':
            ...
            if key == 'forward_safe|straight' and bridge <= float(self.params.turn_bridge_max) and focus <= float(self.params.turn_focus_max):
            ...
            elif key == 'uncertain|none' and bridge <= float(self.params.rescue_bridge_max) and focus >= float(self.params.rescue_focus_min) and path_open >= float(self.params.rescue_path_min) and int(search_state.get('cx32_rescue_count', 0)) < int(self.params.rescue_budget):
                ...
                search_state['cx32_rescue_count'] = int(search_state.get('cx32_rescue_count', 0)) + 1
...
def make_policy(memory: dict[str, Any], params: CX32BBSRParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return BSRPolicy(case, bundle, field, params, memory, disable_bsr=bool(ablation.get('disable_bsr', False)), enable_diagnostics=bool(ablation.get('enable_diagnostics', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX32BBSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX32BBSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

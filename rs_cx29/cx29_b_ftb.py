...
from rs_cx29.common import FAMILY_BUCKETS
...
from rs_cx23.common import apply_class_edit, macros_for_bucket


@dataclass(frozen=True)
class CX29BFTBParams:
...
def param_grid() -> list[CX29BFTBParams]:
    return [
        CX29BFTBParams(2, 18, 0.10, 0.54, 1, 0.15, 0.06, 0.0, 32, 0.02, 0.05, 32, 16, 2, 24, 1),
        CX29BFTBParams(2, 18, 0.10, 0.54, 1, 0.12, 0.05, 0.0, 32, 0.02, 0.05, 32, 16, 2, 24, 2),
        CX29BFTBParams(2, 14, 0.08, 0.50, 1, 0.10, 0.05, 0.0, 28, 0.02, 0.04, 28, 16, 2, 24, 1),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-ForwardTurn-Blend', 'disable_forward_turn_blend': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX29BFTBParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'ftb_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class FTBPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX29BFTBParams, memory: dict[str, Any], disable_forward_turn_blend: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_forward_turn_blend = bool(disable_forward_turn_blend)
    ...
    def _blend_ctx(self, ctx: dict[str, Any]) -> dict[str, Any]:
        out = apply_class_edit(ctx, self.teacher.shadow_teacher.lag_teacher, 'forward_safe|forward_turn', max_macros=int(self.teacher.params.max_macros))
        rules = {fam: 'discouraged' for fam in FAMILY_BUCKETS}
        rules['forward_turn'] = 'allowed'
        rules['straight'] = 'allowed'
        out['rules'] = rules
        conf = {fam: 0.0 for fam in FAMILY_BUCKETS}
        conf['forward_turn'] = 1.0
        conf['straight'] = 0.75
        out['conf'] = conf
        turn_macros = macros_for_bucket(self.teacher.shadow_teacher.lag_teacher, 'forward_safe', 'forward_turn', max_macros=int(self.teacher.params.max_macros))
        straight_macros = macros_for_bucket(self.teacher.shadow_teacher.lag_teacher, 'forward_safe', 'straight', max_macros=int(max(self.params.straight_share, 1)))
        merged = []
        seen = set()
        for macro in list(turn_macros) + list(straight_macros):
            key = tuple(int(i) for i in getattr(macro, 'primitive_indices', ()))
            if key in seen:
                continue
            seen.add(key)
            merged.append(macro)
        out['macros'] = merged[: max(int(self.teacher.params.max_macros), 1) + int(max(self.params.straight_share, 0))]
        out['mode'] = 'forward_safe_blend'
        return out
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if (not self.disable_forward_turn_blend) and str(self.scene_kind) == 'misc' and str(search_state.get('haa_state', 'observe')) in {'candidate', 'commit'}:
...
def make_policy(memory: dict[str, Any], params: CX29BFTBParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return FTBPolicy(
        ...
        params,
        memory,
        disable_forward_turn_blend=bool(ablation.get('disable_forward_turn_blend', False)),
        enable_diagnostics=bool(ablation.get('enable_diagnostics', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX29BFTBParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX29BFTBParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

...
def param_grid() -> list[CX27CEFMParams]:
    return [
        CX27CEFMParams(2, 18, 0.45, 0.12, 0.10, 0.52, 1, 0.02, 0.05, 40, 16, 2, 24),
        CX27CEFMParams(2, 14, 0.40, 0.10, 0.08, 0.50, 1, 0.02, 0.04, 32, 16, 2, 24),
        CX27CEFMParams(1, 14, 0.35, 0.08, 0.06, 0.48, 1, 0.01, 0.04, 28, 12, 2, 20),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Failure-Memory', 'disable_efm': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX27CEFMParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'efm_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class EFMPolicy(CX27DiagnosticsMixin):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX27CEFMParams, memory: dict[str, Any], disable_efm: bool = False, enable_diagnostics: bool = False) -> None:
        self.case = case
        self.bundle = bundle
        self.field = np.asarray(field, dtype=np.float32)
        self.params = params
        self.disable_efm = bool(disable_efm)
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        guard_reason = 'none'
        if not self.disable_efm and str(search_state.get('haa_state', 'observe')) in {'candidate', 'commit'}:
            current_key = str(evidence['class_key'])
            kind = str(evidence['scene_kind'])
            foundation = ctx.get('foundation')
            maze_trigger = bool(
                kind == 'maze'
                and (
                    bool(evidence['blocklist_hit'])
                    or int(evidence['revisit_count']) >= int(self.params.revisit_thr)
            ...
            if maze_trigger and current_key == 'forward_safe|straight':
                if foundation is not None and (
                    ...
                    guard_reason = 'maze_memory_reverse'
                else:
                    ...
                    guard_reason = 'maze_memory_abstain'
            elif misc_trigger and current_key == 'forward_safe|straight':
                if str(search_state.get('haa_state', 'observe')) == 'commit' and not bool(evidence['blocklist_hit']):
                    ...
                    guard_reason = 'misc_memory_soft'
                else:
                    ctx = downgrade_to_uncertain(search_state, ctx, self.teacher.shadow_teacher.lag_teacher, max_macros=int(self.teacher.params.max_macros))
                    guard_reason = 'misc_memory_abstain'
...
def make_policy(memory: dict[str, Any], params: CX27CEFMParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return EFMPolicy(
        ...
        params,
        memory,
        disable_efm=bool(ablation.get('disable_efm', False)),
        enable_diagnostics=bool(ablation.get('enable_diagnostics', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX27CEFMParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX27CEFMParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

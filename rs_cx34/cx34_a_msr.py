...
from rs_cx34.common import CUSTOM_MACRO_REV2, CX34SliceSpec, build_frozen_haa_stack, scene_match


@dataclass(frozen=True)
class CX34AMSRParams:
...
def param_grid() -> list[CX34AMSRParams]:
    return [
        CX34AMSRParams(0.10, 0.36, 0.08, 0.39, 0.99, 1, 0.11, 0.13, 0.31, 0.97, 0.125, 0.34, 0.97, 0.075, 0.095, 0.34, 0.37, 0.97, 1.01, 2, 18, 0.10, 0.54, 0.02, 0.05, 32, 16, 2, 24),
        CX34AMSRParams(0.10, 0.36, 0.08, 0.39, 0.99, 1, 0.11, 0.13, 0.31, 0.97, 0.125, 0.34, 0.97, 0.078, 0.095, 0.34, 0.37, 0.97, 1.01, 2, 18, 0.10, 0.54, 0.02, 0.05, 32, 16, 2, 24),
        CX34AMSRParams(0.10, 0.36, 0.08, 0.39, 0.99, 1, 0.11, 0.13, 0.31, 0.97, 0.125, 0.34, 0.97, 0.075, 0.098, 0.34, 0.38, 0.97, 1.01, 2, 18, 0.10, 0.54, 0.02, 0.05, 32, 16, 2, 24),
    ]
...
def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX34AMSRParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
...
class MSRPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX34AMSRParams, memory: dict[str, Any], disable_macro_rescue: bool = False) -> None:
        self.case = case
        self.bundle = bundle
...
def make_policy(memory: dict[str, Any], params: CX34AMSRParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
...
def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX34AMSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX34AMSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:

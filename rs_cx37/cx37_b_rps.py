...
from rs_cx37.common import build_frozen_haa_stack, compile_replay_trigger_contract, replay_priority_prior


@dataclass(frozen=True)
class CX37BRPSParams:
...
def param_grid() -> list[CX37BRPSParams]:
    return [
        CX37BRPSParams(
            0.10,
            0.36,
...
def ablation_specs() -> list[dict[str, Any]]:
    return [
        {'name': 'No-Replay-Prior', 'disable_replay_prior': True},
        {'name': 'No-Replay-Scheduler', 'disable_replay_scheduler': True},
    ]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX37BRPSParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    parent_params = parent_mod.CX36BETCParams(
    ...
    def _parent_factory(case, bundle, field):
        return parent_mod.make_policy(
            {'haa_teacher': teacher, 'typed_macro_families': typed['families'], 'typed_macro_support': typed['support'], 'event_contract': None},
            parent_params,
            case,
            bundle,
            field,
    ...
    replay_contract = compile_replay_trigger_contract(
        calib_train_assets,
        _parent_factory,
        ...
        out_dir=out_dir / 'replay_contract',
    )
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / 'cx37_b_meta.json').write_text(
        json.dumps(
            {
                'params': params.__dict__,
                'typed_families': {name: {'hits': int(stat.hits), 'avg_score': float(stat.avg_score)} for name, stat in typed['support'].items()},
                'replay_contract': {
                    'positive_hits': int(replay_contract.positive_hits),
                    'negative_hits': int(replay_contract.negative_hits),
                    'margin_floor': float(replay_contract.margin_floor),
                    'high_margin_floor': float(replay_contract.high_margin_floor),
                },
            },
            indent=2,
            ensure_ascii=False,
...
class RPSPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX37BRPSParams, memory: dict[str, Any], disable_replay_prior: bool = False, disable_replay_scheduler: bool = False) -> None:
        self.case = case
        self.bundle = bundle
        self.field = np.asarray(field, dtype=np.float32)
        self.params = params
        self.disable_replay_prior = bool(disable_replay_prior)
        self.disable_replay_scheduler = bool(disable_replay_scheduler)
        ...
        self.base = parent_mod.make_policy(
            {'haa_teacher': memory['haa_teacher'], 'typed_macro_families': self.typed_macro_families, 'typed_macro_support': self.typed_macro_support, 'event_contract': None},
    ...
    def extra_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        if not active or not macros:
            return extra
        node_ctx['_cx37b_review'] = dict(info)
    ...
    def rank_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        if self.disable_replay_scheduler or not isinstance(node_ctx, dict):
            return items
        info = dict(node_ctx.get('_cx37b_review', {}))
        if not info:
            return items
        prior_score = float(info.get('prior_score', 0.0))
        if prior_score <= 0.0:
            return items
        prior_scale = 0.06 * float(min(prior_score, 2.0))
        ranked = []
        for cand, decision in items:
            ...
            if getattr(cand, 'source', 'primitive') == 'macro':
                if not self.disable_replay_prior:
                    dec['priority_secondary_delta'] = float(dec.get('priority_secondary_delta', 0.0)) - prior_scale
                    dec['priority_primary_delta'] = float(dec.get('priority_primary_delta', 0.0)) - 0.5 * prior_scale
            else:
                if not self.disable_replay_prior:
                    dec['priority_secondary_delta'] = float(dec.get('priority_secondary_delta', 0.0)) + 0.25 * prior_scale
...
def make_policy(memory: dict[str, Any], params: CX37BRPSParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return RPSPolicy(
        ...
        params,
        memory,
        disable_replay_prior=bool(ablation.get('disable_replay_prior', False)),
        disable_replay_scheduler=bool(ablation.get('disable_replay_scheduler', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX37BRPSParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX37BRPSParams, memory: dict[str, Any] | None = None) -> np.ndarray:

...
from rs_cx37.common import build_frozen_haa_stack, compile_replay_trigger_contract, replay_trigger_decision


@dataclass(frozen=True)
class CX37ARPTParams:
...
def param_grid() -> list[CX37ARPTParams]:
    return [
        CX37ARPTParams(
            0.10,
            0.36,
...
def ablation_specs() -> list[dict[str, Any]]:
    return [
        {'name': 'No-Replay-Trigger', 'disable_replay_trigger': True},
        {'name': 'No-Replay-Contract', 'disable_replay_contract': True},
    ]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX37ARPTParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'cx37_a_meta.json').write_text(
        json.dumps(
            {
...
class RPTPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX37ARPTParams, memory: dict[str, Any], disable_replay_trigger: bool = False, disable_replay_contract: bool = False) -> None:
        self.case = case
        self.bundle = bundle
        self.field = np.asarray(field, dtype=np.float32)
        self.params = params
        self.disable_replay_trigger = bool(disable_replay_trigger)
        self.disable_replay_contract = bool(disable_replay_contract)
    ...
    def extra_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        if self.disable_replay_trigger or not isinstance(node_ctx, dict):
            return extra
        allow, macros, _ = replay_trigger_decision(
            self.case,
            self.bundle,
            node_ctx,
            search_state,
        ...
        if self.disable_replay_contract and str(class_key(node_ctx)) == 'uncertain|none':
            allow = True
...
def make_policy(memory: dict[str, Any], params: CX37ARPTParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return RPTPolicy(
        ...
        params,
        memory,
        disable_replay_trigger=bool(ablation.get('disable_replay_trigger', False)),
        disable_replay_contract=bool(ablation.get('disable_replay_contract', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX37ARPTParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX37ARPTParams, memory: dict[str, Any] | None = None) -> np.ndarray:

...
def param_grid() -> list[CX19CUGSParams]:
    return [
        CX19CUGSParams(0.24, 0.14, 0.07, 0.02, 0.56, 0.34, 0.16, 2, 2, 0.10, 0.10, 0.12, 0.10, 0.20, 0.20, 0.26, 0.36, 0.24, 0.10, 0.04, 0.08, 2, 2, 5),
        CX19CUGSParams(0.22, 0.12, 0.06, 0.02, 0.54, 0.36, 0.18, 3, 2, 0.12, 0.12, 0.14, 0.12, 0.22, 0.18, 0.28, 0.38, 0.24, 0.10, 0.04, 0.08, 2, 2, 5),
        CX19CUGSParams(0.20, 0.10, 0.05, 0.015, 0.52, 0.38, 0.20, 3, 3, 0.14, 0.14, 0.16, 0.14, 0.24, 0.18, 0.28, 0.40, 0.26, 0.10, 0.04, 0.08, 2, 2, 6),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Unified-Graph', 'disable_substrate': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX19CUGSParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    save_meta(out_dir / 'ugs_meta.json', {'params': params.__dict__, 'state_cfg': state_cfg.__dict__, 'macro_support': serializable_support_state(macro_support), 'graph_support': serializable_support_state(graph_support)})
    return {'viability_table': table, 'macros': macros, 'macro_support': macro_support, 'macro_counts': macro_counts, 'graph': graph, 'graph_support': graph_support, 'best_val_loss': float('nan')}


class UGSPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], params: CX19CUGSParams, memory: dict[str, Any], disable_substrate: bool = False) -> None:
        self.case = case
        self.params = params
    ...
    def rank_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        for cand, stats in zip(candidates, cand_stats):
            ...
            delta -= float(self.params.grammar_bonus) * float(state_family_bonus(state, fam, int(cand.direction)))
...
def make_policy(memory: dict[str, Any], params: CX19CUGSParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return UGSPolicy(case, bundle, params, memory, disable_substrate=bool(ablation.get('disable_substrate', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX19CUGSParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    bundle, field = build_base_field(case, predictor, cfg)
    case['_cx19_bundle'] = bundle
    return field.astype(np.float32)


def build_standard_field(sample, predictor, params: CX19CUGSParams, memory: dict[str, Any] | None = None) -> np.ndarray:

...
def param_grid() -> list[CX19BMCGParams]:
    return [
        CX19BMCGParams(0.24, 0.14, 0.07, 0.02, 0.56, 0.34, 0.16, 2, 0.10, 0.08, 0.22, 0.20, 0.26, 0.36, 0.24, 0.10, 0.04, 0.08, 2, 2, 5),
        CX19BMCGParams(0.22, 0.12, 0.06, 0.02, 0.54, 0.36, 0.18, 2, 0.12, 0.10, 0.24, 0.18, 0.28, 0.38, 0.24, 0.10, 0.04, 0.08, 2, 2, 5),
        CX19BMCGParams(0.20, 0.10, 0.05, 0.015, 0.52, 0.38, 0.20, 3, 0.14, 0.12, 0.26, 0.18, 0.28, 0.40, 0.26, 0.10, 0.04, 0.08, 2, 2, 6),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Compiler-Graph', 'disable_graph': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX19BMCGParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    save_meta(out_dir / 'mcg_meta.json', {'params': params.__dict__, 'state_cfg': state_cfg.__dict__, 'support': serializable_support_state(support), 'graph_sizes': {k: len(v) for k, v in graph.items()}})
    return {'viability_table': table, 'graph': graph, 'support': support, 'best_val_loss': float('nan')}


class MCGPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], params: CX19BMCGParams, memory: dict[str, Any], disable_graph: bool = False) -> None:
        self.case = case
        self.params = params
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        edges = [] if self.disable_graph else choose_state_motif_edges(
            state,
            self.graph,
            self.support,
            feat,
    ...
    def extra_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        if self.disable_graph or not isinstance(node_ctx, dict) or not bool(node_ctx.get('active', False)):
            return []
        macros = []
    ...
    def rank_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        for cand, stats in zip(candidates, cand_stats):
            ...
            delta -= 0.5 * float(self.params.family_bonus) * float(state_family_bonus(state, fam, int(cand.direction)))
...
def make_policy(memory: dict[str, Any], params: CX19BMCGParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return MCGPolicy(case, bundle, params, memory, disable_graph=bool(ablation.get('disable_graph', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX19BMCGParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    bundle, field = build_base_field(case, predictor, cfg)
    case['_cx19_bundle'] = bundle
    return field.astype(np.float32)


def build_standard_field(sample, predictor, params: CX19BMCGParams, memory: dict[str, Any] | None = None) -> np.ndarray:

...
def param_grid() -> list[CX33ADSRParams]:
    return [
        CX33ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'reverse_setup|reverse', 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.13, 0.33, 0.97, 'forward_safe|forward_turn', 0.02, 0.05, 32, 16, 2, 24),
        CX33ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'reverse_setup|reverse', 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.13, 0.33, 0.96, 'forward_safe|forward_turn', 0.02, 0.05, 32, 16, 2, 24),
        CX33ADSRParams(2, 18, 0.10, 0.54, 0.10, 0.36, 0.08, 0.39, 0.99, 'reverse_setup|reverse', 0.11, 0.13, 0.31, 0.97, 'uncertain|none', 0.125, 0.34, 0.97, 'forward_safe|forward_turn', 0.02, 0.05, 32, 16, 2, 24),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Stubborn-Slice', 'disable_stubborn_slice': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX33ADSRParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'cx33_a_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class DSRPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX33ADSRParams, memory: dict[str, Any], disable_stubborn_slice: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_stubborn_slice = bool(disable_stubborn_slice)
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if str(self.case.get('scenario', '')) == 'parasol_misc':
            ...
            elif key == 'uncertain|none' and bridge <= float(self.params.rescue_bridge_max) and focus >= float(self.params.rescue_focus_min) and path_open >= float(self.params.rescue_path_min):
            ...
            elif (not self.disable_stubborn_slice) and key == 'uncertain|none' and bridge >= float(self.params.stubborn_bridge_min) and focus <= float(self.params.stubborn_focus_max) and path_open <= float(self.params.stubborn_path_max):
...
def make_policy(memory: dict[str, Any], params: CX33ADSRParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return DSRPolicy(case, bundle, field, params, memory, disable_stubborn_slice=bool(ablation.get('disable_stubborn_slice', False)), enable_diagnostics=bool(ablation.get('enable_diagnostics', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX33ADSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX33ADSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

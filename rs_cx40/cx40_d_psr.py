...
def param_grid() -> list[CX40DPSRParams]:
    return [
        CX40DPSRParams(**dict(params.__dict__, seed_min_hits=2))
...
def ablation_specs() -> list[dict[str, Any]]:
    return [
        {'name': 'No-Seed-Bank', 'disable_seed_bank': True},
...
def _compile_seed_bank(calib_train_assets: list[dict[str, Any]], *, memory: dict[str, Any], params_obj: CX40DPSRParams) -> dict[tuple[Any, ...], list[SeedBankEntry]]:
    driver_params = parent_mod.CX40ACASParams(**{k: v for k, v in params_obj.__dict__.items() if k != 'seed_min_hits'})
    bank_rows: dict[tuple[Any, ...], dict[str, Any]] = {}
    for asset in calib_train_assets:
        ...
        planner, h_pair = bridge_mod.build_local_proxy(case, field) if hasattr(bridge_mod, 'build_local_proxy') else __import__('rs_cx36.common', fromlist=['build_local_proxy']).build_local_proxy(case, field)
        ...
        for state in path[:-1]:
            ...
            for bridge in depth1_paths:
                bridge_eff = bridge_efficiency(case, h_pair, float(anchor), bridge)
                feat = runtime_bridge_feature(evidence, float(anchor), bridge, bridge_eff=float(bridge_eff), scheduler_score=float(scheduler_score))
                allow, _, _ = prescreener_decision(policy.prescreener, feat, float(scheduler_score))
                if not allow:
                    continue
                sig = seed_bank_signature(
                    case,
                    bundle,
                    ctx,
                    evidence,
                ...
                bucket = bank_rows.setdefault(sig, {'pos': [], 'neg': []})
                ...
                row = (np.asarray(feat, dtype=np.float32), tuple(int(v) for v in macro_indices), float(prior))
                if active:
                    bucket['pos'].append(row)
                else:
                    bucket['neg'].append(row)
            if hasattr(policy, 'complete_expand'):
                policy.complete_expand(planner, rec, tuple(map(float, case['goal'])), None, ctx, 0, 0, 0, search_state, h_pair)

    seed_bank: dict[tuple[Any, ...], list[SeedBankEntry]] = {}
    for sig, bucket in bank_rows.items():
        entries: list[SeedBankEntry] = []
        for active_flag, rows in ((True, bucket['pos']), (False, bucket['neg'])):
            if len(rows) < int(max(params_obj.seed_min_hits, 1)):
                continue
            feat_proto = np.mean(np.stack([row[0] for row in rows], axis=0), axis=0).astype(np.float32)
            if active_flag:
                macro_counts: dict[tuple[int, ...], int] = {}
                prior_sum = 0.0
                for _, macro_indices, prior in rows:
                    macro_counts[tuple(int(v) for v in macro_indices)] = int(macro_counts.get(tuple(int(v) for v in macro_indices), 0)) + 1
                    prior_sum += float(prior)
                macro_indices = max(macro_counts.items(), key=lambda item: int(item[1]))[0]
                entries.append(SeedBankEntry(True, tuple(int(v) for v in macro_indices), float(prior_sum) / float(max(len(rows), 1)), feat_proto, len(rows)))
            else:
                entries.append(SeedBankEntry(False, tuple(), 0.0, feat_proto, len(rows)))
        if entries:
            seed_bank[sig] = entries
    return seed_bank


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX40DPSRParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    seed_bank = _compile_seed_bank(calib_train_assets, memory=memory, params_obj=params)
    out_dir.mkdir(parents=True, exist_ok=True)
    (out_dir / 'cx40_d_meta.json').write_text(
        json.dumps(
            {
                'params': params.__dict__,
                'seed_bank_size': int(sum(len(v) for v in seed_bank.values())),
                'seed_bank_keys': int(len(seed_bank)),
            },
            indent=2,
            ensure_ascii=False,
        ),
...
class PSRPolicy:
    def __init__(
        ...
        bundle: dict[str, Any],
        field: np.ndarray,
        params: CX40DPSRParams,
        memory: dict[str, Any],
        *,
        disable_seed_bank: bool = False,
        ...
        self.params = params
        self.disable_seed_bank = bool(disable_seed_bank)
        ...
        self.seed_bank: dict[tuple[Any, ...], list[SeedBankEntry]] = {} if self.disable_seed_bank else dict(memory.get('seed_bank', {}))
    ...
    def _lookup_seed(self, sig: tuple[Any, ...], feat: np.ndarray) -> SeedBankEntry | None:
        for entry in self.seed_bank.get(sig, []):
            if cache_feature_consistent(MemoizedWitness(entry.active, entry.macro_primitive_indices, entry.prior_score, entry.feature_proto, entry.hits), feat):
                self.stats['seed_hits'] = float(self.stats.get('seed_hits', 0.0) + 1.0)
                return entry
        return None

    def _update_online(self, exact_sig: tuple[Any, ...], active: bool, macro: Any | None, prior: float, feat: np.ndarray) -> None:
        if self.disable_online_refinement:
            return
        self.online_cache[exact_sig] = MemoizedWitness(bool(active), tuple(int(v) for v in getattr(macro, 'primitive_indices', ())) if macro is not None else tuple(), float(prior), np.asarray(feat, dtype=np.float32), 1)
    ...
    def _evaluate_one(
        ...
        *,
        ...
        exact_sig = witness_cache_signature(
            self.case,
            self.bundle,
            node_ctx,
            evidence,
        ...
        cached = self.online_cache.get(exact_sig)
        if cached is not None and cache_feature_consistent(cached, feat):
            ...
            if macro is not None:
                review_rows = bridge_review_candidates(self.case, planner, h_pair, [(bridge, macro, float(cached.prior_score), float(_bridge_review_score(float(current_anchor), bridge)))], source='bridge_review')
                if review_rows:
            ...
            seed_entry = self._lookup_seed(seed_sig, feat)
            if seed_entry is not None:
                if not bool(seed_entry.active):
                    return None
                macro = reconstruct_macro_from_indices(self.typed_macro_families, tuple(int(v) for v in seed_entry.macro_primitive_indices))
                if macro is not None:
                    review_rows = bridge_review_candidates(self.case, planner, h_pair, [(bridge, macro, float(seed_entry.prior_score), float(_bridge_review_score(float(current_anchor), bridge)))], source='bridge_review')
                    if review_rows:
                        final_eff = review_efficiency_for_candidate(self.case, h_pair, float(current_anchor), review_rows[0])
                        if float(final_eff) > float(counterfactual_floor):
                            self.stats['contract_pass'] = float(self.stats.get('contract_pass', 0.0) + 1.0)
                            return bridge, macro, float(seed_entry.prior_score), float(_bridge_review_score(float(current_anchor), bridge))
                        self.stats['contract_reject'] = float(self.stats.get('contract_reject', 0.0) + 1.0)
                        return None
        ...
        if not isinstance(sctx, dict):
            self._update_online(exact_sig, False, None, 0.0, feat)
            return None
        ...
        macro = macros[0]
        prior = float(info.get('compatibility_score', 0.0))
        self._update_online(exact_sig, True, macro, float(prior), feat)
        review_rows = bridge_review_candidates(self.case, planner, h_pair, [(bridge, macro, float(prior), float(_bridge_review_score(float(current_anchor), bridge)))], source='bridge_review')
        if not review_rows:
            return None
        final_eff = review_efficiency_for_candidate(self.case, h_pair, float(current_anchor), review_rows[0])
        if float(final_eff) > float(counterfactual_floor):
...
def make_policy(memory: dict[str, Any], params: CX40DPSRParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return PSRPolicy(
        ...
        params,
        memory,
        disable_seed_bank=bool(ablation.get('disable_seed_bank', False)),
        disable_online_refinement=bool(ablation.get('disable_online_refinement', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX40DPSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    parent_params = parent_mod.CX40ACASParams(**{k: v for k, v in params.__dict__.items() if k != 'seed_min_hits'})
    return parent_mod.build_nonholonomic_field(case, predictor, cfg, parent_params, memory)


def build_standard_field(sample, predictor, params: CX40DPSRParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    parent_params = parent_mod.CX40ACASParams(**{k: v for k, v in params.__dict__.items() if k != 'seed_min_hits'})

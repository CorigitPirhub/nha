...
from rs_cx4.common import ACCEPTED_CX3D_PARAMS, accepted_cx3d_nonholonomic, accepted_cx3d_standard
from scripts.evaluate_baselines import _load_nonholonomic_case, _make_rs_anchor, _path_length, _run_hybrid_method

CX4_MODULES = {
    'CX4-A': 'rs_cx4.cx4_a_pdl',
    'CX4-D': 'rs_cx4.cx4_d_bbi',
    'CX4-C': 'rs_cx4.cx4_c_cbe',
    'CX4-B': 'rs_cx4.cx4_b_ags',
}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX4 main trials on mp/csm/parasol_narrow.')
    ...
    p.add_argument('--variants', type=str, default='CX4-A,CX4-D,CX4-C,CX4-B')
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx4_main_trials_v1'))
...
def _build_dev_cases(dev_files: list[Path], predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig, cap: int) -> list[dict[str, Any]]:
    cases=[]
    for p in dev_files:
...
def _select_variant_config(key: str, predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig, dev_cases: list[dict[str, Any]], cap: int):
    mod = importlib.import_module(CX4_MODULES[key])
    ...
    for params in grid:
        ...
        for item in dev_cases:
            if memory is None:
                field = build_nonh(item['case'], predictor, cfg, params)
            else:
                field = build_nonh(item['case'], predictor, cfg, params, memory)
        ...
        print(f'[rs-p0cx4:{key}] dev params={params.__dict__} score={score}')
        if path_delta and float(np.mean(path_delta)) < -0.75:
            print(f'[rs-p0cx4:{key}] skip params={params.__dict__} because path_delta={float(np.mean(path_delta)):.6f}')
            continue
    ...
    print(f'[rs-p0cx4:{key}] chosen params={chosen["params"]} score={chosen["score"]}')
    return chosen
...
def _eval_hard_variant(key: str, predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig, params: dict[str, Any], memory: Any, parasol_cases: list[dict[str, Any]], cap_exp3: int, cap_exp4: int) -> dict[str, Any]:
    ...
    for i,item in enumerate(parasol_cases, start=1):
        case=item['case']
        if memory is None:
            field=build_nonh(case,predictor,cfg,p_obj)
        else:
            field=build_nonh(case,predictor,cfg,p_obj,memory)
        cx3=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=field),max_expansions=int(cap_exp3))
        cx4=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=field),max_expansions=int(cap_exp4))
        ...
        if i % 5 == 0 or i == total:
            print(f'[rs-p0cx4:{key}] parasol {i}/{total}')
    def agg(rows):
...
def _eval_standard_variant(key: str, predictor: NeuralHeuristicPredictor, params: dict[str, Any], memory: Any, standard_samples: dict[str, list[Any]]):
    mod = importlib.import_module(CX4_MODULES[key])
    ...
    for ds, samples in [('mp',standard_samples['mp']),('csm',standard_samples['csm'])]:
        for i,sample in enumerate(samples, start=1):
            ...
            if memory is None:
                ...
                field=build_std(sample,predictor,p_obj,memory)
            _maybe_cuda_synchronize(predictor)
            ...
            if i % 100 == 0 or i == len(samples):
                print(f'[rs-p0cx4:{key}] standard {ds} {i}/{len(samples)}')
    out={}
    for ds in ['mp','csm']:
...
def main() -> None:
    ...
    print(f'[rs-p0cx4] preload dev cases={len(dev_files)}')
    ...
    print(f'[rs-p0cx4] preload parasol cases={len(parasol_cases)}')
    ...
    for key in _variants(args.variants):
        ...
        report=[f'# {key} P0-CX4 Trial Report','',f'- chosen params: `{chosen["params"]}`','',
        ...
        (args.reports_root / f'rs_p0cx4_{key.lower().replace("-","_")}_main_v1.md').write_text('\n'.join(report),encoding='utf-8')
    ...
    print(f'[rs-p0cx4] summary={args.out_root / "summary.json"}')

...
MODULES = {'CX42-B': 'rs_cx42.cx42_b_qcr'}
SLUG = {'CX42-B': 'b'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX42 round2 pilots.')
    p.add_argument('--variants', type=str, default='CX42-B')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx42b:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx42b:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx42b:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx42b:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx42b_haa_cache')
    ...
    cx41_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx41_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX41-B (Full)' and str(r.get('dataset')) == 'exp4']
    ...
    for variant in variants:
        ...
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx42b:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        chosen = sorted(trials, key=lambda x: x['score'], reverse=True)[0]
        params_obj = chosen['params_obj']; memory = chosen['memory']
        public_rows = []; public_rows.extend(cx3_public_rows); public_rows.extend(parent_public_rows); public_rows.extend(cx41_public_rows)
        ...
        d3 = _delta(s, ('dataset',), 'CX41-B (Full)')
        f2 = _family_delta(public_rows, ('dataset',), 'CX34-A (Full)')
        _write_csv(out_root / 'public_summary.csv', s); _write_csv(out_root / 'public_delta_vs_cx3.csv', d1); _write_csv(out_root / 'public_delta_vs_parent.csv', d2); _write_csv(out_root / 'public_delta_vs_cx41b.csv', d3); _write_csv(out_root / 'public_family_delta_vs_parent.csv', f2)
        ...
        lines = [f'# {variant} Pilot V1', '', '- protocol: frozen `CX34-A` parent; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', f"- chosen params: `{params_obj.__dict__}`", f"- output root: `{out_root}`", '', '## Public vs `CX34-A (Full)`']
        ...
        for row in d3: lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        ...
        summary_rows.append({'variant': variant, 'chosen_params': params_obj.__dict__, 'exp4_exp_delta_vs_parent': float(full2['exp_delta']), 'exp4_runtime_vs_parent': float(full2['mean_time_overhead_ratio']), 'misc_delta_vs_parent': float(misc['exp_delta']), 'maze_delta_vs_parent': float(maze['exp_delta']), 'flange_delta_vs_parent': float(flange['exp_delta']), 'narrow_delta_vs_parent': float(narrow['exp_delta']), 'report_path': str(report_path), 'out_root': str(out_root)})

    summary_root = args.outputs_root / 'rs_p0cx42_round2_summary'; summary_root.mkdir(parents=True, exist_ok=True)
    ...
    lines = ['# P0-CX42 Round2 Summary', '', '- protocol: frozen `CX34-A` parent; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows: lines.append(f"- `{row['variant']}`: vs `CX34-A` exp_delta=`{row['exp4_exp_delta_vs_parent']:.3f}`, runtime_ratio=`{row['exp4_runtime_vs_parent']:.6f}`, misc=`{row['misc_delta_vs_parent']:.3f}`, maze=`{row['maze_delta_vs_parent']:.3f}`, flange=`{row['flange_delta_vs_parent']:.3f}`, narrow=`{row['narrow_delta_vs_parent']:.3f}`")
    (args.reports_root / 'rs_p0cx42_round2_summary.md').write_text('\n'.join(lines), encoding='utf-8')
    audit_lines = ['# P0-CX42 Round2 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx42_round2_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

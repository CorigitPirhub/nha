...
from rs_cx7.common import accepted_bundle_nonholonomic
...
def parse_args():
    p=argparse.ArgumentParser(description='Run P0-CX7 public parasol ablations.')
    ...
    p.add_argument('--chosen-root', type=Path, default=Path('outputs/rs_p0cx7_main_trials_v1'))
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx7_ablation_v1'))
...
def main():
    ...
    parasol_cases=_load_parasol_cases(args.parasol_root); print(f'[rs-p0cx7-ablation] preload parasol cases={len(parasol_cases)}')
    scenario_map={item['path'].name:str(item['case']['scenario']) for item in parasol_cases}
    exp3_frozen=_load_frozen_rows(args.exp3_detail,'exp3',{'Full','No-Residual','No-RS'}); exp4_frozen=_load_frozen_rows(args.exp4_detail,'exp4',{'Hybrid A* (RS)','Ours'}); print(f'[rs-p0cx7-ablation] loaded frozen rows exp3={len(exp3_frozen)} exp4={len(exp4_frozen)}')
    ...
    for key in _variants(args.variants):
        ...
        chosen=json.loads(chosen_path.read_text(encoding='utf-8')); params=chosen['params']; print(f'[rs-p0cx7-ablation:{key}] chosen params={params}')
        mod=importlib.import_module(CX7_MODULES[key]); build_nonh=getattr(mod,'build_nonholonomic_field'); p_obj=type('P',(),params)(); memory=mod.build_dev_memory(dev_cases,predictor,cfg,p_obj) if hasattr(mod,'build_dev_memory') else None
        rows=[]
        ...
        for i,item in enumerate(parasol_cases,start=1):
            case=item['case']; sample_name=item['path'].name; plain_field=_plain_residual_field(case,predictor,cfg); _,cx3d_field=accepted_bundle_nonholonomic(case,predictor,cfg); cx7_field=build_nonh(case,predictor,cfg,p_obj,memory) if memory is not None else build_nonh(case,predictor,cfg,p_obj)
            for budget_name,budget in [('exp3',args.fixed_cap_exp3),('exp4',args.fixed_cap_exp4)]:
                plain=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=plain_field),max_expansions=int(budget)); cx3d=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=cx3d_field),max_expansions=int(budget)); cx7=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=cx7_field),max_expansions=int(budget))
                for method,result in [('Plain-Residual',plain),('CX3-D',cx3d),(key,cx7)]: rows.append({'budget':budget_name,'sample_name':sample_name,'scenario':str(case['scenario']),'method':method,'success':float(result['success']),'expansions':float(result['expansions']),'path_length':float(_path_length(result['path'])) if result['path'] else float('nan'),'time_ms':float(result['runtime_ms'])})
            if i % 5 == 0 or i == total: print(f'[rs-p0cx7-ablation:{key}] parasol {i}/{total}')
        method_summary=[]; family_summary=[]
        ...
        (args.reports_root / f'rs_p0cx7_{key.lower().replace("-","_")}_ablation_v1.md').write_text('\n'.join(report),encoding='utf-8')
        print(f'[rs-p0cx7-ablation] finished {key}')

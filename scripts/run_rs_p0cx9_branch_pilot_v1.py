...
from rs_cx9.common import load_nonholonomic_assets, run_hybrid_with_policy
from rs_cx8.common import write_inputs_sha256

MODULES = {
    'CX9-A': 'rs_cx9.cx9_a_sbm',
    'CX9-D': 'rs_cx9.cx9_d_bwr',
    'CX9-B': 'rs_cx9.cx9_b_csp',
    'CX9-C': 'rs_cx9.cx9_c_cpf',
}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run branch-specific P0-CX9 dev-only pilot.')
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag=f'rs-p0cx9-{args.variant.lower()}:train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.fixed_cap), tag=f'rs-p0cx9-{args.variant.lower()}:val')
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        ...
        for asset in val_assets:
            ...
            policy = mod.make_policy(memory, params_obj, asset['case'], asset['bundle'], field, args.device)
            prep_ms = (time.perf_counter() - prep_t0) * 1000.0
            plan = run_hybrid_with_policy(asset['case'], field, int(args.fixed_cap), successor_policy=policy, record_expanded=False)
            total_cx_time = float(prep_ms + float(plan.runtime_ms))
            ...
            case_rows.append({
                ...
                'time_delta_ms': float(baseline.runtime_ms) - float(total_cx_time),
                'time_overhead_ratio': (float(total_cx_time) - float(baseline.runtime_ms)) / max(float(baseline.runtime_ms), 1e-6),
        ...
        print(f'[rs-p0cx9-{args.variant.lower()}] trial={idx} params={params_obj} summary={val_summary}')
    ...
    if 'train_samples' in chosen['memory']:
        chosen_json['train_samples'] = int(chosen['memory']['train_samples'])
        chosen_json['val_samples'] = int(chosen['memory'].get('val_samples', 0))
    ...
    manifest = {
        'version': 'rs_p0cx9_branch_pilot_v1',
        'variant': args.variant,
    ...
    for row in family_rows:
        lines.append(f"- `{row['scenario']}`: success_delta_pp=`{row['success_delta_pp']:.3f}`, exp_delta=`{row['exp_delta']:.3f}`, time_overhead_ratio=`{row['time_overhead_ratio']:.6f}`, prep_time_ms=`{row['prep_time_ms']:.3f}`")
    lines += ['', '## Readout']
    if float(chosen_json['val_summary']['exp_delta']) > 0.0 and float(chosen_json['val_summary']['time_overhead_ratio']) <= 0.30:
        lines.append('- result: positive pilot trend with controlled runtime overhead')

...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Phase-10 system-level closed-loop validation runner.")
    ...
    p.add_argument("--out-dir", type=Path, default=Path("outputs/router_phase10_system_v1"))
    p.add_argument("--report-md", type=Path, default=Path("reports/router_phase10_system_v1.md"))
    p.add_argument("--episodes-per-platform", type=int, default=240)
    p.add_argument("--episode-quota", type=str, default="mp:150,csm:80,parasol:10")
...
def _simulate_episode(
    ...
    if diff == "hard":
        n_dyn = max(n_dyn - 1, 0)
    # Keep dynamic perturbation present but sparse to avoid degenerate deadlocks in tight maps.
    if n_dyn > 0 and float(rng.random()) > 0.25:
        n_dyn = 0
...
def _write_report(path: Path, stats: dict) -> None:
    lines: list[str] = []
    lines.append("# Router Phase10 System V1 Report")
    lines.append("")
...
def main() -> None:
    ...
    for i_pf, pf in enumerate(_platforms()):
        print(f"[phase10] platform={pf.name}")
        ep_rows: list[dict] = []
        cy_rows: list[dict] = []
        ...
        for i_ep, r in sel.iterrows():
            ...
            if (i_ep + 1) % 20 == 0 or (i_ep + 1) == len(sel):
                ...
                print(f"[phase10] {pf.name} processed {i_ep + 1}/{len(sel)} episodes, success={succ:.4f}")
    ...
    gate = {
        ...
        "episodes_per_platform_ge_200": bool(
            all(int(m["num_episodes"]) >= 200 for m in platform_metrics.values())
        ),
    ...
    stats = {
        "version": "router_phase10_system_v1",
    ...
    print(f"[phase10] stats={stats_path}")
    print(f"[phase10] report={args.report_md}")
    print(f"[phase10] gate={gate}")
    if bool(args.enforce_gate) and not all(gate.values()):
        raise RuntimeError("Phase-10 gate failed. Check outputs/router_phase10_system_v1/stats.json")

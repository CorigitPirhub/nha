...
MODULES = {'CX34-A': 'rs_cx34.cx34_a_msr'}
SLUG = {'CX34-A': 'a'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX34 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX34-A')
...
def _eval_variant(mod, memory, params_obj, predictor, cfg, assets, cap, device, method_name):
    rows = []
    total = len(assets)
...
def _eval_ablation(mod, memory, params_obj, predictor, cfg, assets, cap, device, method_name, ablation):
    rows = []
    total = len(assets)
    for idx, asset in enumerate(assets, start=1):
        field = mod.build_nonholonomic_field(asset['case'], predictor, cfg, params_obj, memory)
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx34:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def _trial_score(parent_rows, trial_rows):
    grouped = defaultdict(list)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx34:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx34:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx34:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx34_haa_cache')
    dependencies = {'haa_teacher': haa_teacher}

    parent_mod = importlib.import_module('rs_cx33.cx33_b_bsr')
    parent_params = parent_mod.CX33BBSRParams(**json.loads(Path('outputs/rs_p0cx33_b_pilot_v1/chosen.json').read_text())['params'])
    parent_val_rows = _eval_variant(parent_mod, {'haa_teacher': haa_teacher}, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX33-B (Parent)')
    parent_val_map = {str(r['sample_name']): dict(r) for r in parent_val_rows}
    parent_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx33_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX33-B (Full)' and str(r.get('dataset')) == 'exp4']
    cx3_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx33_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX3-D' and str(r.get('dataset')) == 'exp4']
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx34_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx34_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx34:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        s=_summary(public_rows,('dataset','method')); d1=_delta(s,('dataset',),'CX3-D'); d2=_delta(s,('dataset',),'CX33-B (Full)'); f1=_family_delta(public_rows,('dataset',),'CX3-D'); f2=_family_delta(public_rows,('dataset',),'CX33-B (Full)')
        _write_csv(out_root/'public_summary.csv',s); _write_csv(out_root/'public_delta_vs_cx3.csv',d1); _write_csv(out_root/'public_delta_vs_parent.csv',d2); _write_csv(out_root/'public_family_delta_vs_cx3.csv',f1); _write_csv(out_root/'public_family_delta_vs_parent.csv',f2)
        ...
        # write report
        lines=[f'# {variant} Pilot V1','', '- protocol: frozen `CX33-B / Budgeted Stubborn-Slice Repair` parent; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit; no hard-test evidence consumed', f"- chosen params: `{params_obj.__dict__}`", f"- output root: `{out_root}`", '', '## Public vs `CX3-D`']
        ...
        lines += ['', '## Public Family Breakdown vs `CX33-B (Full)`']
        ...
        full1=next(r for r in d1 if r['method']==f'{variant} (Full)'); full2=next(r for r in d2 if r['method']==f'{variant} (Full)'); misc=next(r for r in f2 if r['method']==f'{variant} (Full)' and r['scenario']=='parasol_misc'); maze=next(r for r in f2 if r['method']==f'{variant} (Full)' and r['scenario']=='maze'); flange=next(r for r in f2 if r['method']==f'{variant} (Full)' and r['scenario']=='flange'); narrow=next(r for r in f2 if r['method']==f'{variant} (Full)' and r['scenario']=='narrow_passage')
    ...
    summary_root=args.outputs_root/'rs_p0cx34_round1_summary'; summary_root.mkdir(parents=True, exist_ok=True)
    _write_csv(summary_root/'summary.csv', summary_rows); (summary_root/'summary.json').write_text(json.dumps({'variants':summary_rows}, indent=2, ensure_ascii=False), encoding='utf-8')
    lines=['# P0-CX34 Round1 Summary','', '- protocol: frozen `CX33-B / Budgeted Stubborn-Slice Repair` parent; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit; no hard-test evidence consumed','', '## Variant Readout']
    ...
    ranked=sorted(summary_rows,key=lambda r:(r['misc_delta_vs_parent'],r['maze_delta_vs_parent'],r['flange_delta_vs_parent']+r['narrow_delta_vs_parent'],r['exp4_exp_delta_vs_parent']), reverse=True)
    lines += ['', '## Ordering']
    for idx,row in enumerate(ranked,start=1): lines.append(f"- rank {idx}: `{row['variant']}`")
    (args.reports_root/'rs_p0cx34_round1_summary.md').write_text('\\n'.join(lines), encoding='utf-8')
    audit_lines=['# P0-CX34 Standard Audit V1','', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction','']
    ...
    (args.reports_root/'rs_p0cx34_standard_audit_v1.md').write_text('\\n'.join(audit_lines), encoding='utf-8')

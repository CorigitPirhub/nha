...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX47 round5 store-likelihood event proxy dev screen.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx47_e_dev_v1'))
    return p.parse_args()
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev-cap if False else args.dev_cap), tag='cx47e:train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx47e:val')

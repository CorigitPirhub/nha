...
MODULES = {
    'CX28-A': 'rs_cx28.cx28_a_mcr',
    'CX28-B': 'rs_cx28.cx28_b_mcp',
    'CX28-C': 'rs_cx28.cx28_c_msa',
    'CX28-D': 'rs_cx28.cx28_d_mft',
    'CX28-E': 'rs_cx28.cx28_e_bft',
}
SLUG = {'CX28-A': 'a', 'CX28-B': 'b', 'CX28-C': 'c', 'CX28-D': 'd', 'CX28-E': 'e'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX28 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX28-A,CX28-B,CX28-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx28:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append({
            'dataset': dataset,
...
def _trial_score(parent_rows: dict[str, dict[str, Any]], trial_rows: list[dict[str, Any]]) -> tuple:
    ...
    return (
        1 if maze_gain >= -5.0 and flange_gain >= -10.0 and narrow_gain >= -5.0 else 0,
        misc_gain,
        maze_gain,
        flange_gain,
        narrow_gain,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], public_delta_cx3: list[dict[str, Any]], public_family_cx3: list[dict[str, Any]], public_delta_parent: list[dict[str, Any]], public_family_parent: list[dict[str, Any]], standard_rows: list[dict[str, Any]], diag_note: str | None) -> None:
    ...
    lines += ['', '## Public vs `CX27-A (Full)`']
    for row in public_delta_parent:
        lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
    lines += ['', '## Public Family Breakdown vs `CX27-A (Full)`']
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx28:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx28:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx28:public')

    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx28_haa_cache')
    dependencies = {'haa_teacher': haa_teacher}

    parent_mod = importlib.import_module('rs_cx27.cx27_a_mdg')
    parent_params = parent_mod.CX27AMDGParams(**json.loads(Path('outputs/rs_p0cx27_a_pilot_v1/chosen.json').read_text(encoding='utf-8'))['params'])
    parent_val_rows, _ = _eval_variant(parent_mod, {'haa_teacher': haa_teacher}, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX27-A (Parent)', record_diagnostics=False)
    parent_val_map = {str(r['sample_name']): dict(r) for r in parent_val_rows}

    parent_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx27_a_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX27-A (Full)' and str(r.get('dataset')) == 'exp4']
    cx3_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx27_a_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX3-D' and str(r.get('dataset')) == 'exp4']
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx28_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx28_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx28:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_delta_parent = [r for r in _delta(public_summary, ('dataset',), baseline_method='CX27-A (Full)') if r['dataset'] == 'exp4' and str(r['method']).startswith(variant)]
        public_family_parent = [r for r in _family_delta(public_rows, ('dataset',), baseline_method='CX27-A (Full)') if r['dataset'] == 'exp4' and str(r['method']).startswith(variant)]
        ...
        if diag_rows:
            ...
            diag_note = f"diagnostic rows saved to `outputs/rs_p0cx28_{slug}_pilot_v1/diagnostic_rows.csv`; guard_reason_counts=`{dict(reason_counts)}`"
    ...
    summary_root = args.outputs_root / 'rs_p0cx28_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx28_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX28 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx28_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

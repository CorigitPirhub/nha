...
MODULES = {'CX40-A': 'rs_cx40.cx40_a_cas'}
SLUG = {'CX40-A': 'a'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX40 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX40-A')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx40:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx40:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx40:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx40:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx40_haa_cache')

    parent_mod = importlib.import_module('rs_cx39.cx39_c_cbc')
    parent_params = parent_mod.CX39CCBCParams(**json.loads(Path('outputs/rs_p0cx39_c_pilot_v1/chosen.json').read_text(encoding='utf-8'))['params'])
    parent_memory = parent_mod.fit_variant(train_assets, val_contexts, predictor, cfg, parent_params, args.outputs_root / 'parent_fit_cx40', args.device, {'haa_teacher': haa_teacher})
    parent_val_rows = _eval_variant(parent_mod, parent_memory, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX39-C (Parent)')
    ...
    for variant in variants:
        ...
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx40:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_delta_cx3 = _delta(public_summary, ('dataset',), 'CX3-D')
        public_delta_cx36 = _delta(public_summary, ('dataset',), 'CX36-B (Full)')
        public_delta_parent = _delta(public_summary, ('dataset',), 'CX39-C (Full)')
        public_family_parent = _family_delta(public_rows, ('dataset',), 'CX39-C (Full)')
        public_family_cx36 = _family_delta(public_rows, ('dataset',), 'CX36-B (Full)')
        ...
        _write_csv(out_root / 'public_delta_vs_cx39c.csv', public_delta_parent)
        _write_csv(out_root / 'public_family_delta_vs_cx39c.csv', public_family_parent)
        _write_csv(out_root / 'public_family_delta_vs_cx36.csv', public_family_cx36)
        ...
        lines += ['', '## Public Family Breakdown vs `CX39-C (Full)`']
        ...
        lines += ['', '## Public vs `CX36-B (Full)`']
        for row in public_delta_cx36:
            lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        ...
        full_cx36 = next(r for r in public_delta_cx36 if r['method'] == f'{variant} (Full)')
        ...
        summary_rows.append(
            {
                ...
                'exp4_exp_delta_vs_cx39c': float(full_parent['exp_delta']),
                'exp4_runtime_vs_cx39c': float(full_parent['mean_time_overhead_ratio']),
                'exp4_exp_delta_vs_cx36': float(full_cx36['exp_delta']),
                'exp4_runtime_vs_cx36': float(full_cx36['mean_time_overhead_ratio']),
                'misc_delta_vs_cx39c': float(misc['exp_delta']),
                'maze_delta_vs_cx39c': float(maze['exp_delta']),
                'flange_delta_vs_cx39c': float(flange['exp_delta']),
                'narrow_delta_vs_cx39c': float(narrow['exp_delta']),
                'report_path': str(report_path),
                'out_root': str(out_root),
            }
        )

    summary_root = args.outputs_root / 'rs_p0cx40_round1_summary'
    ...
    lines = ['# P0-CX40 Round1 Summary', '', '- protocol: frozen `CX39-C` parent on public evidence; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows:
        lines.append(
            f"- `{row['variant']}`: vs `CX39-C` exp_delta=`{row['exp4_exp_delta_vs_cx39c']:.3f}`, runtime_ratio=`{row['exp4_runtime_vs_cx39c']:.6f}`, vs `CX36-B` exp_delta=`{row['exp4_exp_delta_vs_cx36']:.3f}`, runtime_ratio=`{row['exp4_runtime_vs_cx36']:.6f}`, misc=`{row['misc_delta_vs_cx39c']:.3f}`, maze=`{row['maze_delta_vs_cx39c']:.3f}`, flange=`{row['flange_delta_vs_cx39c']:.3f}`, narrow=`{row['narrow_delta_vs_cx39c']:.3f}`"
        )
    (args.reports_root / 'rs_p0cx40_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX40 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx40_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

...
from rs_cx17.common import run_hybrid_with_policy, standard_identity_error
from rs_cx8.common import load_nonholonomic_assets, load_nonholonomic_contexts, write_inputs_sha256

MODULES = {
    'CX17-A': 'rs_cx17.cx17_a_vml',
    'CX17-B': 'rs_cx17.cx17_b_mag',
    'CX17-C': 'rs_cx17.cx17_c_hps',
}
SLUG = {'CX17-A': 'a', 'CX17-B': 'b', 'CX17-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX17 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX17-A,CX17-B,CX17-C')
...
def _eval_variant(mod, memory: dict[str, Any], params_obj: Any, predictor, cfg: CXGlobalConfig, assets: list[dict[str, Any]], cap: int, device: str, method_name: str) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        bundle = asset['case'].get('_cx17_bundle', asset['bundle'])
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx17:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], val_family_rows: list[dict[str, Any]], public_delta: list[dict[str, Any]], public_family: list[dict[str, Any]], hard_delta: list[dict[str, Any]], hard_family: list[dict[str, Any]], standard_rows: list[dict[str, Any]]) -> None:
    lines = [
        f'# {title}',
        '',
    ...
    if hard_delta:
        ...
        lines.append('- skipped: public positive ceiling gate did not clear, so hard-test evidence was not consumed.')
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx17:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx17:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx17:public')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx17_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx17_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            family_summary = {
                scenario: {'exp_delta': float(np.mean([r['exp_delta'] for r in grp])), 'time_overhead_ratio': float(np.mean([r['time_overhead_ratio'] for r in grp]))}
            ...
            print(f'[cx17:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        public_rows = []
        public_rows.extend(_cached_baseline_rows(args.public_baseline_cache, dataset='exp3'))
        public_rows.extend(_cached_baseline_rows(args.public_baseline_cache, dataset='exp4'))
        ...
        should_run_hard = bool(full_exp4 is not None and flange is not None and float(full_exp4['exp_delta']) > 0.0 and float(flange['exp_delta']) >= 0.0)

        hard_delta: list[dict[str, Any]] = []
        hard_family: list[dict[str, Any]] = []
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx17:{variant}:hard-test')
            hard_rows = []
        ...
        _variant_report(report_path, f'{variant} Pilot V1', chosen_json, val_family_rows, public_delta, public_family, hard_delta, hard_family, standard_rows)
    ...
    summary_root = args.outputs_root / 'rs_p0cx17_round1_summary'
    ...
    lines = [
        '# P0-CX17 Round1 Summary',
        '',
        '- protocol: dev-only selection on `calib_hard_v1`; locked public `parasol_narrow` evaluation; hard-test consumed only when a variant clears the public positive ceiling gate',
        '',
        '## Variant Readout',
    ]
    for row in summary_rows:
    ...
    (args.reports_root / 'rs_p0cx17_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX17 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx17_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

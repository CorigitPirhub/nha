...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run frozen hard-test evaluation for CX41-B.')
    ...
    p.add_argument('--chosen-json', type=Path, default=Path('outputs/rs_p0cx41_b_pilot_v1/chosen.json'))
    p.add_argument('--parent-chosen-json', type=Path, default=Path('outputs/rs_p0cx40_a_pilot_v1/chosen.json'))
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx41_b_hard_eval_v1'))
...
def _eval_cx3(assets: list[dict[str, Any]], cap: int) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        if idx % 5 == 0 or idx == total:
            print(f'[cx41b:hard:CX3-D] {idx}/{total}', flush=True)
    return rows
...
def main() -> None:
    ...
    mod = importlib.import_module('rs_cx41.cx41_b_fdr')
    ...
    params_obj = mod.CX41BFDRParams(**chosen['params'])
    parent_params = parent_mod.CX40ACASParams(**parent_chosen['params'])
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag='cx41b:hard:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx41b:hard:calib-val')
    hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag='cx41b:hard:test')
    ...
    rows.extend(_eval_variant(parent_mod, parent_memory, parent_params, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, 'CX40-A (Full)'))
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, 'CX41-B (Full)'))
    ...
    delta_vs_parent = _delta(summary, ('dataset',), 'CX40-A (Full)')
    family_vs_cx3 = _family_delta(rows, ('dataset',), 'CX3-D')
    family_vs_parent = _family_delta(rows, ('dataset',), 'CX40-A (Full)')
    ...
    _write_csv(args.outputs_root / 'hard_family_delta_vs_cx3.csv', family_vs_cx3)
    _write_csv(args.outputs_root / 'hard_family_delta_vs_parent.csv', family_vs_parent)

    inputs = [args.ours_checkpoint, args.split_root / 'calib_train.csv', args.split_root / 'calib_val.csv', args.chosen_json, args.parent_chosen_json] + train_files + val_files + hard_files
    ...
    lines += ['', '## Hard Family Breakdown vs `CX40-A (Full)`']
    for row in family_vs_parent:
        lines.append(f"- `{row['scenario']}` / `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`, path_delta=`{float(row['path_delta']):.3f}`")
    (args.reports_root / 'rs_p0cx41_b_hard_eval_v1.md').write_text('\n'.join(lines), encoding='utf-8')

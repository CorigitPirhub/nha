...
from rs_cx2.common import CXGlobalConfig, run_standard_astar
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX2 main trials on mp/csm/parasol_narrow.')
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx2_main_trials_v1'))
...
def _select_variant_config(key: str, predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig, dev_cases: list[dict[str, Any]], cap: int):
    ...
    for params in grid:
        ...
        print(f'[rs-p0cx2:{key}] dev params={params.__dict__} score={score}')
        if path_delta and float(np.mean(path_delta)) < -1.0:
            print(f'[rs-p0cx2:{key}] skip params={params.__dict__} because path_delta={float(np.mean(path_delta)):.6f}')
            continue
    ...
    print(f'[rs-p0cx2:{key}] chosen params={chosen["params"]} score={chosen["score"]}')
    return chosen
...
def _eval_hard_variant(key: str, predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig, params: dict[str, Any], parasol_cases: list[dict[str, Any]], cap_exp3: int, cap_exp4: int) -> dict[str, Any]:
    ...
    for i, item in enumerate(parasol_cases, start=1):
        ...
        if i % 5 == 0 or i == total:
            print(f'[rs-p0cx2:{key}] parasol {i}/{total}')
    ...
    for ds, samples in [('mp', standard_samples['mp']), ('csm', standard_samples['csm'])]:
        for i, sample in enumerate(samples, start=1):
            ...
            if i % 100 == 0 or i == len(samples):
                print(f'[rs-p0cx2:{key}] standard {ds} {i}/{len(samples)}')
    out = {}
    for ds in ['mp', 'csm']:
...
def main() -> None:
    ...
    for key in _variants(args.variants):
        ...
        (args.reports_root / f'rs_p0cx2_{key.lower().replace("-", "_")}_main_v1.md').write_text('\n'.join(report), encoding='utf-8')
    ...
    print(f'[rs-p0cx2] summary={args.out_root / "summary.json"}')

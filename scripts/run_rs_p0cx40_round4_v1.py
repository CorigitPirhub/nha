...
MODULES = {'CX40-D': 'rs_cx40.cx40_d_psr'}
SLUG = {'CX40-D': 'd'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX40 round4 pilots.')
    p.add_argument('--variants', type=str, default='CX40-D')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx40d:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx40d:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx40d:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx40d:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx40d_haa_cache')
    ...
    parent_memory = parent_mod.fit_variant(train_assets, val_contexts, predictor, cfg, parent_params, args.outputs_root / 'parent_fit_cx40d', args.device, {'haa_teacher': haa_teacher})
    ...
    for variant in variants:
        ...
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx40d:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
    ...
    summary_root = args.outputs_root / 'rs_p0cx40_round4_summary'
    ...
    lines = ['# P0-CX40 Round4 Summary', '', '- protocol: frozen `CX40-A` parent on public evidence; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows:
    ...
    (args.reports_root / 'rs_p0cx40_round4_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX40 Round4 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx40_round4_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

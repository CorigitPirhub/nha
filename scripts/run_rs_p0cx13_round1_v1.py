...
MODULES = {
    'CX13-A': 'rs_cx13.cx13_a_bbc',
    'CX13-B': 'rs_cx13.cx13_b_ias',
    'CX13-C': 'rs_cx13.cx13_c_tcb',
}
SLUG = {'CX13-A': 'a', 'CX13-B': 'b', 'CX13-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX13 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX13-A,CX13-B,CX13-C')
    ...
    p.add_argument('--base-public-rows', type=Path, default=Path('outputs/rs_p0cx12_a_pilot_v1/public_case_rows.csv'))
...
def _eval_variant(mod, memory: dict[str, Any], params_obj: Any, predictor, cfg: CXGlobalConfig, contexts: list[dict[str, Any]], cap: int, device: str, method_name: str) -> list[dict[str, Any]]:
    rows = []
    total = len(contexts)
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx13:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx13:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx13:calib-val')
    guard_assets = load_nonholonomic_assets(guard_files, predictor, cfg, int(args.dev_cap), tag='cx13:guard-train')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx13:public-contexts')

    base_public_rows = _read_csv(args.base_public_rows)
    base_public_by_dataset = defaultdict(list)
    for row in base_public_rows:
        if str(row['method']) in {'Hybrid A* (RS)', 'CX3-D'}:
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx13_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx13_{slug}_pilot_v1.md'
        deps = {'guard_assets': guard_assets}
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx13:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        for dataset, cap in [('exp3', int(args.exp3_cap)), ('exp4', int(args.exp4_cap))]:
            variant_rows = _eval_variant(mod, chosen_memory, chosen_params, predictor, cfg, public_contexts, cap, args.device, f'{variant} (Full)')
            ...
            for spec in getattr(mod, 'ablation_specs', lambda: [])():
                ...
                for idx, asset in enumerate(public_contexts, start=1):
                    ...
                    if idx % 5 == 0 or idx == len(public_contexts):
                        print(f"[{variant} ({spec['name']})] {idx}/{len(public_contexts)}")
    ...
    summary_root = args.outputs_root / 'rs_p0cx13_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx13_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

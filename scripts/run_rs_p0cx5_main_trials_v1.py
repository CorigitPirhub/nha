...
from rs_cx5.common import accepted_bundle_nonholonomic, accepted_bundle_standard
from scripts.evaluate_baselines import _load_nonholonomic_case, _make_rs_anchor, _path_length, _run_hybrid_method

CX5_MODULES = {
    'CX5-A': 'rs_cx5.cx5_a_tau',
    'CX5-B': 'rs_cx5.cx5_b_cts',
    'CX5-C': 'rs_cx5.cx5_c_paa',
    'CX5-D': 'rs_cx5.cx5_d_gro',
}


def parse_args() -> argparse.Namespace:
    p=argparse.ArgumentParser(description='Run P0-CX5 main trials on mp/csm/parasol_narrow.')
    ...
    p.add_argument('--variants', type=str, default='CX5-A,CX5-B,CX5-C,CX5-D')
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx5_main_trials_v1'))
...
def _build_dev_cases(dev_files:list[Path], predictor, cfg, cap:int)->list[dict[str,Any]]:
    cases=[]
    for p in dev_files:
        ...
        cases.append({'case':case,'path':p,'cx3d':cx3d})
    return cases


def _load_parasol_cases(root: Path)->list[dict[str,Any]]:
    return [{'path':p,'case':_load_nonholonomic_case(p)} for p in sorted(root.glob('sample_*.npz'))]


def _load_standard_samples(root: Path, max_mp:int, max_csm:int)->dict[str,list[Any]]:
    return {'mp':[load_grid_sample(p) for p in sorted((root/'mp'/'test').glob('sample_*.npz'))[:int(max_mp)]], 'csm':[load_grid_sample(p) for p in sorted((root/'csm'/'test').glob('sample_*.npz'))[:int(max_csm)]]}


def _select_variant_config(key:str, predictor, cfg, dev_cases, cap:int):
    mod=importlib.import_module(CX5_MODULES[key])
    ...
    for params in grid:
        ...
        for item in dev_cases:
            ...
            if item['cx3d']['path'] and cx['path']:
                path_delta.append(float(_path_length(item['cx3d']['path']))-float(_path_length(cx['path'])))
        score=(float(np.mean(success)),float(np.mean(exp_delta)),float(np.mean(time_delta)))
        print(f'[rs-p0cx5:{key}] dev params={params.__dict__} score={score}')
        if path_delta and float(np.mean(path_delta)) < -0.75:
            print(f'[rs-p0cx5:{key}] skip params={params.__dict__} because path_delta={float(np.mean(path_delta)):.6f}')
            continue
        if best is None or score > best['score']:
            best={'params':params.__dict__,'score':score,'memory':memory}
    chosen=best if best is not None else {'params':grid[0].__dict__,'score':(0.0,0.0,0.0),'memory':None}
    print(f'[rs-p0cx5:{key}] chosen params={chosen["params"]} score={chosen["score"]}')
    return chosen


def _eval_hard_variant(key:str, predictor, cfg, params:dict[str,Any], memory:Any, parasol_cases, cap3:int, cap4:int):
    mod=importlib.import_module(CX5_MODULES[key])
    build_nonh=getattr(mod,'build_nonholonomic_field')
    p_obj=type('P',(),params)()
    exp3_rows=[]; exp4_rows=[]
    ...
    for i,item in enumerate(parasol_cases, start=1):
        ...
        if i % 5 == 0 or i == total:
            print(f'[rs-p0cx5:{key}] parasol {i}/{total}')
    def agg(rows):
...
def _eval_standard_variant(key:str, predictor, params:dict[str,Any], memory:Any, standard_samples):
    mod=importlib.import_module(CX5_MODULES[key]); build_std=getattr(mod,'build_standard_field'); p_obj=type('P',(),params)()
    rows=[]
    for ds,samples in [('mp',standard_samples['mp']),('csm',standard_samples['csm'])]:
        for i,sample in enumerate(samples, start=1):
            ...
            if i % 100 == 0 or i == len(samples):
                print(f'[rs-p0cx5:{key}] standard {ds} {i}/{len(samples)}')
    out={}
    for ds in ['mp','csm']:
...
def main() -> None:
    ...
    print(f'[rs-p0cx5] preload dev cases={len(dev_files)}')
    dev_cases=_build_dev_cases(dev_files,predictor,cfg,args.fixed_cap_exp3)
    parasol_cases=_load_parasol_cases(args.parasol_root)
    print(f'[rs-p0cx5] preload parasol cases={len(parasol_cases)}')
    ...
    for key in _variants(args.variants):
        ...
        report=[f'# {key} P0-CX5 Trial Report','',f'- chosen params: `{chosen["params"]}`','', '## Parasol Exp3 vs Frozen Baselines',f"- CX success: `{hard_eval['exp3']['success_rate']:.6f}` vs Full `{float(baseline['exp3_full']['success_rate']):.6f}` vs No-Residual `{float(baseline['exp3_no_res']['success_rate']):.6f}`",f"- CX expansions: `{hard_eval['exp3']['avg_expansions']:.3f}` vs Full `{float(baseline['exp3_full']['avg_expansions']):.3f}` vs No-Residual `{float(baseline['exp3_no_res']['avg_expansions']):.3f}`",f"- CX time: `{hard_eval['exp3']['avg_time_ms']:.3f}` vs Full `{float(baseline['exp3_full']['avg_time_ms']):.3f}` vs No-Residual `{float(baseline['exp3_no_res']['avg_time_ms']):.3f}`",'', '## Parasol Exp4 vs Frozen Baselines',f"- CX success: `{hard_eval['exp4']['success_rate']:.6f}` vs Hybrid `{float(baseline['exp4_hybrid']['success_rate']):.6f}` vs Full `{float(baseline['exp4_full']['success_rate']):.6f}`",f"- CX expansions: `{hard_eval['exp4']['avg_expansions']:.3f}` vs Hybrid `{float(baseline['exp4_hybrid']['avg_expansions']):.3f}` vs Full `{float(baseline['exp4_full']['avg_expansions']):.3f}`",f"- CX time: `{hard_eval['exp4']['avg_time_ms']:.3f}` vs Hybrid `{float(baseline['exp4_hybrid']['avg_time_ms']):.3f}` vs Full `{float(baseline['exp4_full']['avg_time_ms']):.3f}`"]
        if std_eval:
        ...
        (args.reports_root / f'rs_p0cx5_{key.lower().replace("-","_")}_main_v1.md').write_text('\n'.join(report),encoding='utf-8')
    ...
    print(f'[rs-p0cx5] summary={args.out_root / "summary.json"}')

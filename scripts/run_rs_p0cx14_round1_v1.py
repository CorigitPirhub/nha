...
from rs_cx14.common import compare_plan_to_baseline, run_hybrid_with_policy, standard_identity_error
from rs_cx8.common import load_nonholonomic_assets, write_inputs_sha256

MODULES = {
    'CX14-A': 'rs_cx14.cx14_a_nsg',
    'CX14-B': 'rs_cx14.cx14_b_lhu',
    'CX14-C': 'rs_cx14.cx14_c_mhq',
}
SLUG = {'CX14-A': 'a', 'CX14-B': 'b', 'CX14-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX14 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX14-A,CX14-B,CX14-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx14:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx14:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx14:calib-val')
    public_assets_exp3 = load_nonholonomic_assets(public_files, predictor, cfg, int(args.exp3_cap), tag='cx14:public-exp3')
    public_assets_exp4 = load_nonholonomic_assets(public_files, predictor, cfg, int(args.exp4_cap), tag='cx14:public-exp4')

    summary_rows = []
    for variant in variants:
        mod = importlib.import_module(MODULES[variant])
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx14_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx14_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx14:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        for dataset, assets, cap in [('exp3', public_assets_exp3, int(args.exp3_cap)), ('exp4', public_assets_exp4, int(args.exp4_cap))]:
            ...
            public_rows.extend(baseline_rows)
            ...
            for spec in getattr(mod, 'ablation_specs', lambda: [])():
                ...
                for idx, asset in enumerate(assets, start=1):
                    ...
                    bundle = asset['case'].get('_cx14_bundle', asset['bundle'])
                    prep_t0 = time.perf_counter()
                    policy = mod.make_policy(chosen_memory, chosen_params, asset['case'], bundle, field, args.device, ablation=spec)
                    prep_ms = (time.perf_counter() - prep_t0) * 1000.0
                    plan = run_hybrid_with_policy(asset['case'], field, int(cap), successor_policy=policy, record_expanded=False)
                    ab_rows.append({
                        'sample_name': str(asset['path'].name),
    ...
    summary_root = args.outputs_root / 'rs_p0cx14_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx14_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

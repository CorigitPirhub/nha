...
from rs_cx16.common import run_hybrid_with_policy, standard_identity_error
from rs_cx8.common import load_nonholonomic_assets, load_nonholonomic_contexts, write_inputs_sha256

MODULES = {
    'CX16-B': 'rs_cx16.cx16_b_vgo',
    'CX16-C': 'rs_cx16.cx16_c_blr',
    'CX16-D': 'rs_cx16.cx16_d_mec',
    'CX16-A': 'rs_cx16.cx16_a_nml',
    'CX16-E': 'rs_cx16.cx16_e_psr',
}
SLUG = {'CX16-A': 'a', 'CX16-B': 'b', 'CX16-C': 'c', 'CX16-D': 'd', 'CX16-E': 'e'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX16 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX16-B,CX16-C,CX16-D,CX16-A,CX16-E')
...
def _eval_variant(mod, memory: dict[str, Any], params_obj: Any, predictor, cfg: CXGlobalConfig, assets: list[dict[str, Any]], cap: int, device: str, method_name: str) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        bundle = asset['case'].get('_cx16_bundle', asset['bundle'])
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx16:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def _variant_report(
    ...
    if full is None or flange is None:
        ...
        passed = float(full['exp_delta']) > 0.0 and float(full['mean_time_overhead_ratio']) < 0.30 and float(flange['exp_delta']) >= 0.0
        lines.append('- result: public gate cleared.' if passed else '- result: public gate not cleared under the locked protocol.')
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx16:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx16:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx16:public')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx16_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx16_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx16:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        should_run_hard = bool(full_exp4 is not None and flange is not None and float(full_exp4['exp_delta']) > 0.0 and float(full_exp4['mean_time_overhead_ratio']) < 0.30 and float(flange['exp_delta']) >= 0.0)

        hard_delta: list[dict[str, Any]] = []
        hard_family: list[dict[str, Any]] = []
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx16:{variant}:hard-test')
            hard_rows = []
    ...
    summary_root = args.outputs_root / 'rs_p0cx16_round1_summary'
    ...
    lines = [
        '# P0-CX16 Round1 Summary',
        '',
        '- protocol: dev-only selection on `calib_hard_v1`; locked public `parasol_narrow` evaluation; hard-test consumed only when a variant clears the public gate',
        '',
        '## Variant Readout',
    ]
    for row in summary_rows:
    ...
    (args.reports_root / 'rs_p0cx16_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX16 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx16_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

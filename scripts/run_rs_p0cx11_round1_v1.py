...
MODULES = {
    'CX11-B': 'rs_cx11.cx11_b_lds',
    'CX11-C': 'rs_cx11.cx11_c_csv',
    'CX11-A': 'rs_cx11.cx11_a_rst',
}
SLUG = {
    'CX11-B': 'b',
    'CX11-C': 'c',
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX11 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX11-B,CX11-C,CX11-A')
    ...
    p.add_argument('--teacher-chosen-json', type=Path, default=TEACHER_CHOSEN_JSON)
...
def _base_reference_rows(path: Path, dataset: str) -> list[dict[str, Any]]:
    with path.open('r', newline='', encoding='utf-8') as f:
        rows = list(csv.DictReader(f))
    return [r for r in rows if str(r['dataset']) == dataset and str(r['method']) in {'Hybrid A* (RS)', 'CX3-D', 'CX10-D (Full)'}]


def _guard_train_files(split_root: Path, guard_dev_root: Path) -> list[Path]:
    train = set(_read_split_csv(split_root / 'calib_train.csv'))
    heldout = set(_read_split_csv(split_root / 'calib_val.csv'))
    extra = {p for p in sorted(guard_dev_root.glob('sample_*.npz')) if p not in heldout}
    return sorted(train | extra)


def _build_guard_cache(assets: list[dict[str, Any]], base_memory: dict[str, Any], base_params: cx10_d_las.CX10DLASParams, cap: int, device: str) -> list[dict[str, Any]]:
    rows = []
    total = len(assets)
    for idx, asset in enumerate(assets, start=1):
        ctx = proposal_context(base_memory, base_params, asset['case'], asset['bundle'], asset['field'], device)
        full_policy = ctx['base_policy']
        prep_t0 = time.perf_counter()
        plan = run_hybrid_with_policy(asset['case'], asset['field'], int(cap), successor_policy=full_policy, record_expanded=False) if full_policy is not None else asset['baseline_result']
        prep_ms = (time.perf_counter() - prep_t0) * 1000.0 if full_policy is not None else 0.0
        delta = compare_plan_to_baseline(asset['baseline_result'], plan, prep_ms=0.0)
        gate_rows = []
        for gate in ctx['gates']:
            progress_inner = best_mode_progress(asset['case'], asset['field'], gate['state'], int(gate.get('inner_mode', 0)))
            progress_outer = best_mode_progress(asset['case'], asset['field'], gate['state'], int(gate.get('outer_mode', 0))) if int(gate.get('outer_mode', 0)) > 0 else -1e6
            gate_rows.append({
                **gate,
                'mode_progress': float(max(progress_inner, progress_outer)),
            })
        rows.append({
            'sample_name': str(asset['path'].name),
            'scenario': str(asset['case']['scenario']),
            'proposal_feature': np.asarray(ctx['proposal_feature'], dtype=np.float32),
            'num_gates': int(len(ctx['gates'])),
            'gate_rows': gate_rows,
            'success_delta': float(delta.success_delta),
            'exp_delta': float(delta.exp_delta),
            'time_delta_ms': float(delta.time_delta_ms),
            'time_overhead_ratio': float(delta.time_overhead_ratio),
            'path_delta': float(delta.path_delta),
        })
        if idx % 5 == 0 or idx == total:
            print(f'[cx11:guard-cache] {idx}/{total}')
    return rows
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx11:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    calib_train_files = _read_split_csv(args.split_root / 'calib_train.csv')
    calib_val_files = _read_split_csv(args.split_root / 'calib_val.csv')
    guard_train_files = _guard_train_files(args.split_root, args.guard_dev_root)
    public_files = sorted(args.parasol_root.glob('sample_*.npz'))

    train_assets = load_nonholonomic_assets(calib_train_files, predictor, cfg, int(args.dev_cap), tag='cx11:calib-train')
    val_assets = load_nonholonomic_assets(calib_val_files, predictor, cfg, int(args.dev_cap), tag='cx11:calib-val')
    guard_assets = load_nonholonomic_assets(guard_train_files, predictor, cfg, int(args.dev_cap), tag='cx11:guard-train')
    public_assets_exp3 = load_nonholonomic_assets(public_files, predictor, cfg, int(args.exp3_cap), tag='cx11:public-exp3')
    public_assets_exp4 = load_nonholonomic_assets(public_files, predictor, cfg, int(args.exp4_cap), tag='cx11:public-exp4')

    teacher_chosen_json = Path(args.teacher_chosen_json)
    from rs_cx10.common import load_teacher_memory
    teacher_memory = load_teacher_memory(teacher_chosen_json, args.device)
    ...
    base_memory = cx10_d_las.fit_variant(
        ...
        cfg,
        base_params,
        args.outputs_root / 'rs_p0cx11_base_refit',
        args.device,
        dependencies={'teacher_memory': teacher_memory, 'teacher_chosen_json': teacher_chosen_json},
    )
    guard_cache = _build_guard_cache(guard_assets, base_memory, base_params, int(args.dev_cap), args.device)
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx11_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx11_{slug}_pilot_v1.md'
        deps = {
        ...
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx11:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        inputs += calib_train_files + calib_val_files + guard_train_files + public_files
        write_inputs_sha256(inputs, out_root / 'inputs_sha256.json')
        manifest = {
            'version': 'rs_p0cx11_round1_v1',
    ...
    summary_root = args.outputs_root / 'rs_p0cx11_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx11_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

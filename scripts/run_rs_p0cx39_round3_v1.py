...
MODULES = {'CX39-C': 'rs_cx39.cx39_c_cbc'}
SLUG = {'CX39-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX39 round3 pilots.')
    p.add_argument('--variants', type=str, default='CX39-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx39c:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx39c:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx39c:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx39c:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx39c_haa_cache')

    parent_mod = importlib.import_module('rs_cx36.cx36_b_etc')
    parent_params = parent_mod.CX36BETCParams(**json.loads(Path('outputs/rs_p0cx36_rerun_v2/rs_p0cx36_b_pilot_v1/chosen.json').read_text(encoding='utf-8'))['params'])
    parent_memory = parent_mod.fit_variant(train_assets, val_contexts, predictor, cfg, parent_params, args.outputs_root / 'parent_fit_cx39c', args.device, {'haa_teacher': haa_teacher})
    parent_val_rows = _eval_variant(parent_mod, parent_memory, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX36-B (Parent)')
    parent_public_rows = _eval_variant(parent_mod, parent_memory, parent_params, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX36-B (Full)')
    ...
    cx39b_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx39_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) in {'CX39-B (Full)'}]
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx39_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx39_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx39c:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_rows.extend(cx39b_public_rows)
        ...
        public_delta_parent = _delta(public_summary, ('dataset',), 'CX36-B (Full)')
        public_delta_b = _delta(public_summary, ('dataset',), 'CX39-B (Full)')
        public_family_cx3 = _family_delta(public_rows, ('dataset',), 'CX3-D')
        public_family_parent = _family_delta(public_rows, ('dataset',), 'CX36-B (Full)')
        public_family_b = _family_delta(public_rows, ('dataset',), 'CX39-B (Full)')
        ...
        _write_csv(out_root / 'public_delta_vs_cx39b.csv', public_delta_b)
        ...
        _write_csv(out_root / 'public_family_delta_vs_cx39b.csv', public_family_b)
        ...
        for row in public_delta_parent:
            lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        lines += ['', '## Public vs `CX39-B (Full)`']
        for row in public_delta_b:
            lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        lines += ['', '## Public Family Breakdown vs `CX36-B (Full)`']
        ...
        lines += ['', '## Public Family Breakdown vs `CX39-B (Full)`']
        for row in public_family_b:
            lines.append(f"- `{row['scenario']}` / `{row['method']}`: exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        ...
        full_cx3 = next(r for r in public_delta_cx3 if r['method'] == f'{variant} (Full)')
        full_parent = next(r for r in public_delta_parent if r['method'] == f'{variant} (Full)')
        full_b = next(r for r in public_delta_b if r['method'] == f'{variant} (Full)')
        ...
        summary_rows.append({'variant': variant, 'chosen_params': params_obj.__dict__, 'exp4_exp_delta_vs_cx3': float(full_cx3['exp_delta']), 'exp4_overhead_vs_cx3': float(full_cx3['mean_time_overhead_ratio']), 'exp4_exp_delta_vs_parent': float(full_parent['exp_delta']), 'exp4_exp_delta_vs_cx39b': float(full_b['exp_delta']), 'misc_delta_vs_parent': float(misc['exp_delta']), 'maze_delta_vs_parent': float(maze['exp_delta']), 'flange_delta_vs_parent': float(flange['exp_delta']), 'narrow_delta_vs_parent': float(narrow['exp_delta']), 'report_path': str(report_path), 'out_root': str(out_root)})

    summary_root = args.outputs_root / 'rs_p0cx39_round3_summary'
    ...
    lines = ['# P0-CX39 Round3 Summary', '', '- protocol: frozen `CX36-B` parent on public evidence; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows:
        lines.append(f"- `{row['variant']}`: exp4 vs `CX3-D` exp_delta=`{row['exp4_exp_delta_vs_cx3']:.3f}`, overhead=`{row['exp4_overhead_vs_cx3']:.6f}`, vs parent exp_delta=`{row['exp4_exp_delta_vs_parent']:.3f}`, vs `CX39-B` exp_delta=`{row['exp4_exp_delta_vs_cx39b']:.3f}`, misc=`{row['misc_delta_vs_parent']:.3f}`, maze=`{row['maze_delta_vs_parent']:.3f}`, flange=`{row['flange_delta_vs_parent']:.3f}`, narrow=`{row['narrow_delta_vs_parent']:.3f}`")
    (args.reports_root / 'rs_p0cx39_round3_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX39 Round3 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx39_round3_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

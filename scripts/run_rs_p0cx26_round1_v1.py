...
from rs_cx26.common import build_frozen_cx25_stack, dto_contract_meta
...
MODULES = {
    'CX26-A': 'rs_cx26.cx26_a_hst',
    'CX26-B': 'rs_cx26.cx26_b_mgi',
    'CX26-C': 'rs_cx26.cx26_c_tdc',
}
SLUG = {'CX26-A': 'a', 'CX26-B': 'b', 'CX26-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX26 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX26-A,CX26-B,CX26-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx26:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append({
            'dataset': dataset,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], val_family_rows: list[dict[str, Any]], public_delta: list[dict[str, Any]], public_family: list[dict[str, Any]], standard_rows: list[dict[str, Any]], *, observatory_note: str | None = None) -> None:
    ...
    if observatory_note:
        lines += ['', '## DTO Contract', f"- contract saved to `outputs/{Path(chosen_json['out_root']).name}/dto_contract.json`", '', '## Observatory', f'- {observatory_note}']
...
def main() -> None:
    ...
    print(f'[cx26] variants={variants} public_datasets={public_datasets}', flush=True)
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx26:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx26:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx26:public')
    print(f'[cx26] loaded assets train={len(train_assets)} val={len(val_contexts)} public={len(public_contexts)}', flush=True)

    stack = build_frozen_cx25_stack(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx26_stack_cache')
    dependencies = {'cx25_stack': stack}
    print('[cx26] frozen CX25 stack ready', flush=True)

    summary_rows = []
    standard_audit_rows = []

    for variant in variants:
        print(f'[cx26] start variant={variant}', flush=True)
        mod = importlib.import_module(MODULES[variant])
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx26_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx26_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx26:{variant}] trial={idx} params={params_obj} summary={val_summary}', flush=True)

        chosen = _choose_trial(trials)
        chosen_params = chosen['params_obj']
        chosen_memory = chosen['memory']
        (out_root / 'dto_contract.json').write_text(json.dumps(dto_contract_meta(), indent=2, ensure_ascii=False), encoding='utf-8')
        ...
        if diagnostic_rows:
            ...
            for row in diagnostic_rows:
                key_counts[str(row.get('auto_state', 'observe'))] += 1
            observatory_note = f"diagnostic rows saved to `outputs/rs_p0cx26_{slug}_pilot_v1/diagnostic_rows.csv`; state_counts=`{dict(key_counts)}`"
    ...
    summary_root = args.outputs_root / 'rs_p0cx26_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx26_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX26 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx26_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

...
from rs_cx22.common import build_frozen_teacher
...
MODULES = {
    'CX22-A': 'rs_cx22.cx22_a_sdt',
    'CX22-B': 'rs_cx22.cx22_b_dcg',
    'CX22-C': 'rs_cx22.cx22_c_hpg',
    'CX22-D': 'rs_cx22.cx22_d_sha',
}
SLUG = {'CX22-A': 'a', 'CX22-B': 'b', 'CX22-C': 'c', 'CX22-D': 'd'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX22 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX22-A,CX22-B,CX22-C,CX22-D')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx22:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append({
            'dataset': dataset,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], val_family_rows: list[dict[str, Any]], public_delta: list[dict[str, Any]], public_family: list[dict[str, Any]], hard_delta: list[dict[str, Any]], hard_family: list[dict[str, Any]], standard_rows: list[dict[str, Any]], *, hard_skipped_note: str | None = None) -> None:
    ...
    if hard_delta:
        ...
        lines.append(f"- {hard_skipped_note}" if hard_skipped_note else '- skipped: public gate did not clear, so hard-test evidence was not consumed.')
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx22:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx22:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx22:public')

    teacher = build_frozen_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx22_teacher_cache')
    dependencies = {'teacher': teacher}
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx22_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx22_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx22:{variant}] trial={idx} params={params_obj} summary={val_summary}', flush=True)
        ...
        default_run_hard = bool(full_exp4 is not None and flange is not None and float(full_exp4['exp_delta']) > 0.0 and float(flange['exp_delta']) >= 0.0)
        if variant in {'CX22-C', 'CX22-D'}:
            if hasattr(mod, 'should_run_hard'):
                should_run_hard = bool(mod.should_run_hard(public_delta, public_family, f'{variant} (Full)'))
            else:
                should_run_hard = bool(default_run_hard)
        else:
            should_run_hard = False

        hard_delta: list[dict[str, Any]] = []
        hard_family: list[dict[str, Any]] = []
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx22:{variant}:hard-test')
            hard_rows = []
            cached_hard = _cached_baseline_rows(args.hard_baseline_cache, dataset=None)
            for row in cached_hard:
                item = dict(row)
                item['dataset'] = 'hard_test'
        ...
        hard_skipped_note = 'skipped: this round keeps hard-test for `CX22-C/D` only; `CX22-A/B` stay public-first.' if variant in {'CX22-A', 'CX22-B'} else 'skipped: frozen public gate did not clear, so hard-test evidence was not consumed.'
    ...
    summary_root = args.outputs_root / 'rs_p0cx22_round1_summary'
    ...
    lines = [
        ...
        '',
        '- protocol: dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation; hard-test consumed only when each frozen public gate cleared',
        '',
        '## Variant Readout',
    ]
    for row in summary_rows:
    ...
    (args.reports_root / 'rs_p0cx22_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX22 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx22_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

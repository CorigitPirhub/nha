...
def parse_args() -> argparse.Namespace:
    p=argparse.ArgumentParser(description='Run P0-CX4 public parasol ablations.')
    ...
    p.add_argument('--chosen-root', type=Path, default=Path('outputs/rs_p0cx4_main_trials_v1'))
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx4_ablation_v1'))
...
def _plain_residual_field(case: dict[str, Any], predictor: NeuralHeuristicPredictor, cfg: CXGlobalConfig) -> np.ndarray:
...
def main() -> None:
    ...
    print(f'[rs-p0cx4-ablation] preload parasol cases={len(parasol_cases)}')
    ...
    print(f'[rs-p0cx4-ablation] loaded frozen rows exp3={len(exp3_frozen)} exp4={len(exp4_frozen)}')
    dev_files=_hard_dev_files(args.hard_benchmark_root,_families(args.families),args.dev_per_family)
    dev_cases=[{'path':p,'case':_load_nonholonomic_case(p)} for p in dev_files]
    ...
    for key in _variants(args.variants):
        ...
        print(f'[rs-p0cx4-ablation:{key}] chosen params={params}')
        mod=importlib.import_module(CX4_MODULES[key])
        build_nonh=getattr(mod,'build_nonholonomic_field')
        p_obj=type('P', (), params)()
        memory=mod.build_dev_memory(dev_cases,predictor,cfg,p_obj) if hasattr(mod,'build_dev_memory') else None
        rows=[]
        ...
        for i,item in enumerate(parasol_cases, start=1):
            case=item['case']; sample_name=item['path'].name
            plain_field=_plain_residual_field(case,predictor,cfg)
            cx3d_field=build_cx3d_nonholonomic(case,predictor,cfg,ACCEPTED_CX3D_PARAMS)
            cx4_field=build_nonh(case,predictor,cfg,p_obj,memory) if memory is not None else build_nonh(case,predictor,cfg,p_obj)
            for budget_name,budget in [('exp3',args.fixed_cap_exp3),('exp4',args.fixed_cap_exp4)]:
                ...
                cx3d=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=cx3d_field),max_expansions=int(budget))
                cx4=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=cx4_field),max_expansions=int(budget))
                for method,result in [('Plain-Residual',plain),('CX3-D',cx3d),(key,cx4)]:
            ...
            if i % 5 == 0 or i == total:
                print(f'[rs-p0cx4-ablation:{key}] parasol {i}/{total}')
        method_summary=[]; family_summary=[]
        ...
        report=[f'# {key} Ablation Report','',f'- chosen params: `{params}`','',
                '## EXP3',
                f"- {key} success=`{exp3_cx['success_rate']:.6f}` vs CX3-D=`{exp3_cx3d['success_rate']:.6f}` vs Plain=`{exp3_plain['success_rate']:.6f}` vs Full=`{exp3_full['success_rate']:.6f}`",
                f"- {key} expansions=`{exp3_cx['avg_expansions']:.3f}` vs CX3-D=`{exp3_cx3d['avg_expansions']:.3f}` vs Plain=`{exp3_plain['avg_expansions']:.3f}` vs Full=`{exp3_full['avg_expansions']:.3f}`",
                f"- {key} time_ms=`{exp3_cx['avg_time_ms']:.3f}` vs CX3-D=`{exp3_cx3d['avg_time_ms']:.3f}` vs Plain=`{exp3_plain['avg_time_ms']:.3f}`",
                '', '## EXP4',
                f"- {key} success=`{exp4_cx['success_rate']:.6f}` vs CX3-D=`{exp4_cx3d['success_rate']:.6f}` vs Plain=`{exp4_plain['success_rate']:.6f}` vs Ours=`{exp4_ours['success_rate']:.6f}`",
                f"- {key} expansions=`{exp4_cx['avg_expansions']:.3f}` vs CX3-D=`{exp4_cx3d['avg_expansions']:.3f}` vs Plain=`{exp4_plain['avg_expansions']:.3f}` vs Ours=`{exp4_ours['avg_expansions']:.3f}`",
                f"- {key} time_ms=`{exp4_cx['avg_time_ms']:.3f}` vs CX3-D=`{exp4_cx3d['avg_time_ms']:.3f}` vs Plain=`{exp4_plain['avg_time_ms']:.3f}`",
               ]
        (args.reports_root / f'rs_p0cx4_{key.lower().replace("-","_")}_ablation_v1.md').write_text('\n'.join(report),encoding='utf-8')
        print(f'[rs-p0cx4-ablation] finished {key}')

...
from rs_cx.common import fuse_nonholonomic, nonholonomic_base_and_correction
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX3 public parasol ablations against frozen official baselines.')
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx3_ablation_v1'))
...
def main() -> None:
    ...
    print(f'[rs-p0cx3-ablation] preload parasol cases={len(parasol_cases)}')
    ...
    print(f'[rs-p0cx3-ablation] loaded frozen rows exp3={len(exp3_frozen)} exp4={len(exp4_frozen)}')
    ...
    for key in _variants(args.variants):
        ...
        print(f'[rs-p0cx3-ablation:{key}] chosen params={params}')
        ...
        for i, item in enumerate(parasol_cases, start=1):
            ...
            if i % 5 == 0 or i == total:
                print(f'[rs-p0cx3-ablation:{key}] parasol {i}/{total}')
        method_summary=[]; family_summary=[]
        ...
        report=[
            f'# {key} Ablation Report','',f'- chosen params: `{params}`','',
            '## EXP3',
        ...
        (args.reports_root / f'rs_p0cx3_{key.lower().replace("-", "_")}_ablation_v1.md').write_text('\n'.join(report), encoding='utf-8')
        summary.append({'key': key, 'params': params, 'exp3_plain_exp_delta': float(exp3_plain['avg_expansions']) - float(exp3_cx['avg_expansions']), 'exp4_plain_exp_delta': float(exp4_plain['avg_expansions']) - float(exp4_cx['avg_expansions']), 'exp4_success_delta_vs_plain': float(exp4_cx['success_rate']) - float(exp4_plain['success_rate'])})
        print(f'[rs-p0cx3-ablation] finished {key}')
    (args.out_root/'summary.json').write_text(json.dumps({'variants': summary}, indent=2, ensure_ascii=False), encoding='utf-8')

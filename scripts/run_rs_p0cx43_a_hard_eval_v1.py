...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run frozen hard-test evaluation for CX43-A.')
    ...
    p.add_argument('--chosen-json', type=Path, default=Path('outputs/rs_p0cx43_a_pilot_v1/chosen.json'))
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx43_a_hard_eval_v1'))
...
def _eval_cx3(assets: list[dict[str, Any]], cap: int):
    rows = []
    total = len(assets)
...
def main() -> None:
    ...
    params_obj = mod.CX43AARRParams(**chosen['params'])
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag='cx43:hard:train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx43:hard:val')
    hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag='cx43:hard:test')
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, 'CX43-A (Full)'))
    ...
    (args.reports_root / 'rs_p0cx43_a_hard_eval_v1.md').write_text('\n'.join(lines), encoding='utf-8')

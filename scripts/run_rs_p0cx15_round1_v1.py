...
from rs_cx15.common import run_hybrid_with_policy, standard_identity_error
from rs_cx8.common import load_nonholonomic_assets, load_nonholonomic_contexts, write_inputs_sha256

MODULES = {
    'CX15-A': 'rs_cx15.cx15_a_rmc',
    'CX15-B': 'rs_cx15.cx15_b_emr',
    'CX15-C': 'rs_cx15.cx15_c_fme',
    'CX15-D': 'rs_cx15.cx15_d_cbr',
}
SLUG = {'CX15-A': 'a', 'CX15-B': 'b', 'CX15-C': 'c', 'CX15-D': 'd'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX15 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX15-A,CX15-B,CX15-C,CX15-D')
...
def _eval_variant(mod, memory: dict[str, Any], params_obj: Any, predictor, cfg: CXGlobalConfig, assets: list[dict[str, Any]], cap: int, device: str, method_name: str) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        bundle = asset['case'].get('_cx15_bundle', asset['bundle'])
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx15:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx15:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx15:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx15:public')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx15_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx15_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx15:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        public_rows = []
        cached_exp3 = _cached_baseline_rows(args.public_baseline_cache, dataset='exp3')
        cached_exp4 = _cached_baseline_rows(args.public_baseline_cache, dataset='exp4')
        public_rows.extend(cached_exp3)
        public_rows.extend(cached_exp4)
        ...
        should_run_hard = bool(
            full_exp4 is not None
            and flange is not None
            and float(full_exp4['exp_delta']) > 0.0
            and float(full_exp4['mean_time_overhead_ratio']) < 0.30
            and float(flange['exp_delta']) >= 0.0
        )

        hard_delta: list[dict[str, Any]] = []
        hard_family: list[dict[str, Any]] = []
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx15:{variant}:hard-test')
            hard_rows = []
        ...
        val_family_rows = [
            {'scenario': scenario, 'exp_delta': float(vals['exp_delta']), 'time_overhead_ratio': float(vals['time_overhead_ratio'])}
    ...
    summary_root = args.outputs_root / 'rs_p0cx15_round1_summary'
    ...
    for row in summary_rows:
        lines.append(
            f"- `{row['variant']}`: calib_val exp_delta=`{row['calib_val_exp_delta']:.3f}`, calib_val overhead=`{row['calib_val_overhead']:.6f}`, exp4 exp_delta=`{row['exp4_exp_delta']:.3f}`, exp4 overhead=`{row['exp4_overhead']:.6f}`, flange exp_delta=`{row['flange_exp_delta']:.3f}`, hard_escalated=`{int(bool(row['hard_escalated']))}`"
        )
    ...
    (args.reports_root / 'rs_p0cx15_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = [
        '# P0-CX15 Standard Audit V1',
        '',
    ...
    for row in standard_audit_rows:
        audit_lines.append(
            f"- `{row['variant']}` / `{row['dataset']}`: num_cases=`{row['num_cases']}`, max_abs_field_diff=`{float(row['max_abs_field_diff']):.6f}`, mean_abs_field_diff=`{float(row['mean_abs_field_diff']):.6f}`"
        )
    (args.reports_root / 'rs_p0cx15_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

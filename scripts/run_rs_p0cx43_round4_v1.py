...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX43 round4 pre-gated structural rank release pilots.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx43_d_pilot_v1'))
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx43d:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main() -> None:
    ...
    print(f'[cx43d] load assets train={len(train_files)} val={len(val_files)} public={len(public_files)}', flush=True)
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx43d:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx43d:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx43d:public')
    ...
    print('[cx43d] eval parent on val', flush=True)
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        print(f'[cx43d] trial {idx} fit/eval', flush=True)
        ...
        trial_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX43-D (Candidate)')
        ...
        print(f'[cx43d] trial={idx} score={score:.3f} success_gain={success_gain:.6f} exp_gain={exp_gain:.3f} time_gain={time_gain:.3f} params={params_obj}', flush=True)
    chosen = sorted(trials, key=lambda item: item['score'], reverse=True)[0]
    params_obj = chosen['params_obj']
    memory = chosen['memory']

    print('[cx43d] unified public eval', flush=True)
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX43-D (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, f"CX43-D ({spec['name']})", ablation=spec))
    ...
    print('[cx43d] standard audit', flush=True)
    ...
    (args.outputs_root / 'chosen.json').write_text(json.dumps({'variant': 'CX43-D', 'params': params_obj.__dict__, 'selection_score': float(chosen['score']), 'val_success_gain': float(chosen['success_gain']), 'val_exp_gain': float(chosen['exp_gain']), 'val_time_gain': float(chosen['time_gain'])}, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    (args.reports_root / 'rs_p0cx43_d_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

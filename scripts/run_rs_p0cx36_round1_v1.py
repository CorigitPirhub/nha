...
MODULES = {'CX36-A': 'rs_cx36.cx36_a_stc', 'CX36-B': 'rs_cx36.cx36_b_etc'}
SLUG = {'CX36-A': 'a', 'CX36-B': 'b'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX36 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX36-A,CX36-B')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx36:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx36:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx36:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx36:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx36_haa_cache')

    parent_mod = importlib.import_module('rs_cx35.cx35_b_tfmr')
    parent_params = parent_mod.CX35BTFMRParams(**json.loads(Path('outputs/rs_p0cx35_rerun_v2/rs_p0cx35_b_pilot_v1/chosen.json').read_text(encoding='utf-8'))['params'])
    parent_val_rows = _eval_variant(parent_mod, {'haa_teacher': haa_teacher, 'typed_macro_families': (), 'typed_macro_support': {}}, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX35-B (Parent)')
    parent_val_map = {str(r['sample_name']): dict(r) for r in parent_val_rows}

    parent_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx35_rerun_v2/rs_p0cx35_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX35-B (Full)' and str(r.get('dataset')) == 'exp4']
    cx3_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx35_rerun_v2/rs_p0cx35_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX3-D' and str(r.get('dataset')) == 'exp4']
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx36_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx36_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            trials.append({'params_obj': params_obj, 'memory': memory, 'score': float(np.mean([float(parent_val_map[str(r['sample_name'])]['expansions']) - float(r['expansions']) for r in trial_rows])) if trial_rows else 0.0})
            print(f'[cx36:{variant}] trial={idx} params={params_obj} score={trials[-1]["score"]}', flush=True)
        ...
        public_delta_parent = _delta(public_summary, ('dataset',), 'CX35-B (Full)')
        public_family_cx3 = _family_delta(public_rows, ('dataset',), 'CX3-D')
        public_family_parent = _family_delta(public_rows, ('dataset',), 'CX35-B (Full)')
        ...
        inputs = [args.ours-checkpoint if False else args.ours_checkpoint, args.split_root / 'calib_train.csv', args.split_root / 'calib_val.csv'] + train_files + val_files + public_files
        ...
        lines += ['', '## Public vs `CX35-B (Full)`']
        for row in public_delta_parent:
            lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
        lines += ['', '## Public Family Breakdown vs `CX35-B (Full)`']
        ...
        summary_rows.append({'variant': variant, 'chosen_params': params_obj.__dict__, 'exp4_exp_delta_vs_cx3': float(full_cx3['exp_delta']), 'exp4_overhead_vs_cx3': float(full_cx3['mean_time_overhead_ratio']), 'exp4_exp_delta_vs_parent': float(full_parent['exp_delta']), 'misc_delta_vs_parent': float(misc['exp_delta']), 'maze_delta_vs_parent': float(maze['exp_delta']), 'flange_delta_vs_parent': float(flange['exp_delta']), 'narrow_delta_vs_parent': float(narrow['exp_delta']), 'report_path': str(report_path), 'out_root': str(out_root)})

    summary_root = args.outputs_root / 'rs_p0cx36_round1_summary'
    ...
    lines = ['# P0-CX36 Round1 Summary', '', '- protocol: frozen `CX35-B` parent on public evidence; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows:
    ...
    (args.reports_root / 'rs_p0cx36_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX36 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx36_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

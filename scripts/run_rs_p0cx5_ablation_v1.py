...
from rs_cx5.common import accepted_bundle_nonholonomic
...
def parse_args() -> argparse.Namespace:
    p=argparse.ArgumentParser(description='Run P0-CX5 public parasol ablations.')
    ...
    p.add_argument('--chosen-root', type=Path, default=Path('outputs/rs_p0cx5_main_trials_v1'))
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx5_ablation_v1'))
...
def _plain_residual_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig) -> np.ndarray:
...
def _load_parasol_cases(root: Path):
    return [{'path':p,'case':_load_nonholonomic_case(p)} for p in sorted(root.glob('sample_*.npz'))]


def _load_frozen_rows(path: Path, budget_name: str, allowed_methods: set[str]):
...
def main():
    ...
    print(f'[rs-p0cx5-ablation] preload parasol cases={len(parasol_cases)}')
    ...
    print(f'[rs-p0cx5-ablation] loaded frozen rows exp3={len(exp3_frozen)} exp4={len(exp4_frozen)}')
    ...
    for p in dev_files:
        ...
        cx3d=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=field),max_expansions=7000)
        dev_cases.append({'path':p,'case':case,'cx3d':cx3d})
    for key in _variants(args.variants):
        ...
        chosen=json.loads(chosen_path.read_text(encoding='utf-8')); params=chosen['params']
        print(f'[rs-p0cx5-ablation:{key}] chosen params={params}')
        mod=importlib.import_module(CX5_MODULES[key]); build_nonh=getattr(mod,'build_nonholonomic_field'); p_obj=type('P',(),params)()
        ...
        for i,item in enumerate(parasol_cases, start=1):
            ...
            cx5_field=build_nonh(case,predictor,cfg,p_obj,memory) if memory is not None else build_nonh(case,predictor,cfg,p_obj)
            for budget_name,budget in [('exp3',args.fixed_cap_exp3),('exp4',args.fixed_cap_exp4)]:
                ...
                cx5=_run_hybrid_method(case,_make_rs_anchor(case,rs_field=cx5_field),max_expansions=int(budget))
                for method,result in [('Plain-Residual',plain),('CX3-D',cx3d),(key,cx5)]:
            ...
            if i % 5 == 0 or i == total:
                print(f'[rs-p0cx5-ablation:{key}] parasol {i}/{total}')
        method_summary=[]; family_summary=[]
        ...
        (args.reports_root / f'rs_p0cx5_{key.lower().replace("-","_")}_ablation_v1.md').write_text('\n'.join(report),encoding='utf-8')
        print(f'[rs-p0cx5-ablation] finished {key}')

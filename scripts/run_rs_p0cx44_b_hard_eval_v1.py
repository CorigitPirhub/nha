...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run frozen hard-test evaluation for CX44-B.')
    ...
    p.add_argument('--chosen-json', type=Path, default=Path('outputs/rs_p0cx44_b_pilot_v1/chosen.json'))
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx44_b_hard_eval_v1'))
...
def _eval_cx3(assets: list[dict[str, Any]], cap: int) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        if idx % 5 == 0 or idx == total:
            print(f'[cx44b-hard:CX3-D] {idx}/{total}', flush=True)
    return rows
...
def main() -> None:
    ...
    params_obj = mod.CX44BFCWTParams(**chosen['params'])
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag='cx44b-hard:train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx44b-hard:val')
    hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag='cx44b-hard:test')
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, 'CX44-B (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, f"CX44-B ({spec['name']})", ablation=spec))
    ...
    (args.reports_root / 'rs_p0cx44_b_hard_eval_v1.md').write_text('\n'.join(lines), encoding='utf-8')

...
from rs_cx20.common import run_hybrid_with_policy, standard_identity_error
from rs_cx8.common import load_nonholonomic_assets, load_nonholonomic_contexts, write_inputs_sha256

MODULES = {
    'CX20-A': 'rs_cx20.cx20_a_mvf',
    'CX20-B': 'rs_cx20.cx20_b_cmg',
    'CX20-C': 'rs_cx20.cx20_c_csg',
}
SLUG = {'CX20-A': 'a', 'CX20-B': 'b', 'CX20-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX20 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX20-A,CX20-B,CX20-C')
...
def _eval_variant(mod, memory: dict[str, Any], params_obj: Any, predictor, cfg: CXGlobalConfig, assets: list[dict[str, Any]], cap: int, device: str, method_name: str) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        bundle = asset['case'].get('_cx20_bundle', asset['bundle'])
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx20:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx20:calib-train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.dev_cap), tag='cx20:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx20:public')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx20_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx20_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx20:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx20:{variant}:hard-test')
            hard_rows = []
    ...
    summary_root = args.outputs_root / 'rs_p0cx20_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx20_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX20 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx20_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

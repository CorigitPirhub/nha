...
MODULES = {
    'CX33-A': 'rs_cx33.cx33_a_dsr',
    'CX33-B': 'rs_cx33.cx33_b_bsr',
}
SLUG = {'CX33-A': 'a', 'CX33-B': 'b'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX33 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX33-A,CX33-B')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx33:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], public_delta_cx3: list[dict[str, Any]], public_family_cx3: list[dict[str, Any]], public_delta_parent: list[dict[str, Any]], public_family_parent: list[dict[str, Any]], standard_rows: list[dict[str, Any]]) -> None:
    lines = [f'# {title}', '', '- protocol: frozen `CX32-B / Budgeted Slice Repair` parent; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit; no hard-test evidence consumed', f"- chosen params: `{chosen_json['params']}`", f"- output root: `{chosen_json['out_root']}`", '', '## Public vs `CX3-D`']
    ...
    lines += ['', '## Public vs `CX32-B (Full)`']
    for row in public_delta_parent:
        lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
    lines += ['', '## Public Family Breakdown vs `CX32-B (Full)`']
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx33:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx33:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx33:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx33_haa_cache')
    dependencies = {'haa_teacher': haa_teacher}

    parent_mod = importlib.import_module('rs_cx32.cx32_b_bsr')
    parent_params = parent_mod.CX32BBSRParams(**json.loads(Path('outputs/rs_p0cx32_b_pilot_v1/chosen.json').read_text())['params'])
    parent_val_rows = _eval_variant(parent_mod, {'haa_teacher': haa_teacher}, parent_params, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX32-B (Parent)')
    parent_val_map = {str(r['sample_name']): dict(r) for r in parent_val_rows}
    parent_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx32_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX32-B (Full)' and str(r.get('dataset')) == 'exp4']
    cx3_public_rows = [r for r in _read_csv(Path('outputs/rs_p0cx32_b_pilot_v1/public_case_rows.csv')) if str(r.get('method')) == 'CX3-D' and str(r.get('dataset')) == 'exp4']
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx33_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx33_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx33:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_delta_parent = [r for r in _delta(public_summary, ('dataset',), baseline_method='CX32-B (Full)') if r['dataset']=='exp4' and str(r['method']).startswith(variant)]
        public_family_parent = [r for r in _family_delta(public_rows, ('dataset',), baseline_method='CX32-B (Full)') if r['dataset']=='exp4' and str(r['method']).startswith(variant)]
        ...
        for row in standard_rows: standard_rows_all.append({'variant': variant, **row})
        ...
        summary_rows.append({'variant':variant,'chosen_params':params_obj.__dict__,'exp4_exp_delta_vs_cx3':float(full_cx3['exp_delta']) if full_cx3 else float('nan'),'exp4_overhead_vs_cx3':float(full_cx3['mean_time_overhead_ratio']) if full_cx3 else float('nan'),'exp4_exp_delta_vs_parent':float(full_parent['exp_delta']) if full_parent else float('nan'),'misc_delta_vs_parent':float(misc_parent['exp_delta']) if misc_parent else float('nan'),'maze_delta_vs_parent':float(maze_parent['exp_delta']) if maze_parent else float('nan'),'flange_delta_vs_parent':float(flange_parent['exp_delta']) if flange_parent else float('nan'),'narrow_delta_vs_parent':float(narrow_parent['exp_delta']) if narrow_parent else float('nan'),'report_path':str(report_path),'out_root':str(out_root)})

    summary_root=args.outputs_root/'rs_p0cx33_round1_summary'; summary_root.mkdir(parents=True, exist_ok=True)
    ...
    lines=['# P0-CX33 Round1 Summary','','- protocol: frozen `CX32-B / Budgeted Slice Repair` parent; dev-only trial selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit; no hard-test evidence consumed','','## Variant Readout']
    for row in summary_rows:
        lines.append(f"- `{row['variant']}`: exp4 vs `CX3-D` exp_delta=`{float(row['exp4_exp_delta_vs_cx3']):.3f}`, overhead=`{float(row['exp4_overhead_vs_cx3']):.6f}`, vs parent exp_delta=`{float(row['exp4_exp_delta_vs_parent']):.3f}`, misc=`{float(row['misc_delta_vs_parent']):.3f}`, maze=`{float(row['maze_delta_vs_parent']):.3f}`, flange=`{float(row['flange_delta_vs_parent']):.3f}`, narrow=`{float(row['narrow_delta_vs_parent']):.3f}`")
    ...
    lines += ['', '## Ordering']
    for idx,row in enumerate(ranked, start=1): lines.append(f"- rank {idx}: `{row['variant']}`")
    (args.reports_root/'rs_p0cx33_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')
    audit_lines=['# P0-CX33 Standard Audit V1','','- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction','']
    ...
    (args.reports_root/'rs_p0cx33_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

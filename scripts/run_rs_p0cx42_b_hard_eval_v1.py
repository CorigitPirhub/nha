...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run frozen hard-test evaluation for CX42-B.')
    ...
    p.add_argument('--chosen-json', type=Path, default=Path('outputs/rs_p0cx42_b_pilot_v1/chosen.json'))
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx42_b_hard_eval_v1'))
...
def _eval_cx3(assets: list[dict[str, Any]], cap: int) -> list[dict[str, Any]]:
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        if idx % 5 == 0 or idx == total:
            print(f'[cx42b:hard:CX3-D] {idx}/{total}', flush=True)
    return rows
...
def _eval_variant(mod, memory, params_obj, predictor, cfg, assets, cap, device, method_name):
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        rows.append({'dataset': 'hard_test', 'sample_name': str(asset['path'].name), 'scenario': str(asset['case']['scenario']), 'method': method_name, 'success': float(plan.success), 'expansions': float(plan.expansions), 'time_ms': float(plan.runtime_ms + prep_ms), 'path_length': float(_path_len(plan))})
        if idx % 5 == 0 or idx == total:
...
def main() -> None:
    ...
    main_mod = importlib.import_module('rs_cx42.cx42_b_qcr')
    ...
    params_obj = main_mod.CX42BQCRParams(**chosen['params'])
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag='cx42b:hard:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx42b:hard:calib-val')
    hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag='cx42b:hard:test')
    ...
    memory = main_mod.fit_variant(train_assets, val_contexts, predictor, cfg, params_obj, args.outputs_root / 'fit', args.device, {'haa_teacher': teacher})
    ...
    rows.extend(_eval_variant(main_mod, memory, params_obj, predictor, cfg, hard_contexts, int(args.fixed_cap), args.device, 'CX42-B (Full)'))
    ...
    delta_vs_parent = _delta(summary, ('dataset',), 'CX34-A (Full)')
    delta_vs_compat = _delta(summary, ('dataset',), 'CX41-B (Full)')
    family_vs_parent = _family_delta(rows, ('dataset',), 'CX34-A (Full)')
    _write_csv(args.outputs_root / 'hard_summary.csv', summary)
    _write_csv(args.outputs_root / 'hard_delta_vs_cx3.csv', delta_vs_cx3)
    _write_csv(args.outputs_root / 'hard_delta_vs_parent.csv', delta_vs_parent)
    _write_csv(args.outputs_root / 'hard_delta_vs_cx41b.csv', delta_vs_compat)
    _write_csv(args.outputs_root / 'hard_family_delta_vs_parent.csv', family_vs_parent)
    inputs = [args.ours_checkpoint, args.split_root / 'calib_train.csv', args.split_root / 'calib_val.csv', args.chosen_json, args.parent_chosen_json, args.compat_chosen_json] + train_files + val_files + hard_files
    ...
    for row in delta_vs_parent:
        lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`, path_delta=`{float(row['path_delta']):.3f}`")
    lines += ['', '## Hard Benchmark vs `CX41-B (Full)`']
    for row in delta_vs_compat:
        lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`, path_delta=`{float(row['path_delta']):.3f}`")
    lines += ['', '## Hard Family Breakdown vs `CX34-A (Full)`']
    for row in family_vs_parent:
    ...
    (args.reports_root / 'rs_p0cx42_b_hard_eval_v1.md').write_text('\n'.join(lines), encoding='utf-8')

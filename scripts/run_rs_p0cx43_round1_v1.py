...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX43 round1 unified pilots.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx43_a_pilot_v1'))
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx43:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main() -> None:
    ...
    print('[cx43] boot', flush=True)

    mod = importlib.import_module('rs_cx43.cx43_a_arr')
    ...
    print(f'[cx43] load assets train={len(train_files)} val={len(val_files)} public={len(public_files)}', flush=True)
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx43:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx43:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx43:public')
    print('[cx43] build frozen teacher', flush=True)
    ...
    print('[cx43] fit parent/compat', flush=True)
    ...
    print('[cx43] eval parent on val', flush=True)
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        print(f'[cx43] trial {idx} fit start', flush=True)
        ...
        print(f'[cx43] trial {idx} val eval', flush=True)
        trial_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX43-A (Candidate)')
        ...
        print(f'[cx43] trial={idx} score={score:.3f} success_gain={success_gain:.6f} exp_gain={exp_gain:.3f} time_gain={time_gain:.3f} params={params_obj}', flush=True)
    chosen = sorted(trials, key=lambda item: item['score'], reverse=True)[0]
    params_obj = chosen['params_obj']
    memory = chosen['memory']

    print('[cx43] unified public eval', flush=True)
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX43-A (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, f"CX43-A ({spec['name']})", ablation=spec))
    ...
    print('[cx43] standard audit', flush=True)
    ...
    chosen_json = {
        ...
        'val_success_gain': float(chosen['success_gain']),
        'val_exp_gain': float(chosen['exp_gain']),
        'val_time_gain': float(chosen['time_gain']),
    }
    (args.outputs_root / 'chosen.json').write_text(json.dumps(chosen_json, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    (args.reports_root / 'rs_p0cx43_a_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

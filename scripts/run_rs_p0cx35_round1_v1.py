...
MODULES = {'CX35-A': 'rs_cx35.cx35_a_tmfi', 'CX35-B': 'rs_cx35.cx35_b_tfmr', 'CX35-C': 'rs_cx35.cx35_c_tmw'}
SLUG = {'CX35-A': 'a', 'CX35-B': 'b', 'CX35-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX35 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX35-A,CX35-B,CX35-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx35:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append(
            {
                'dataset': dataset,
...
def _trial_score(parent_rows, trial_rows):
    ...
    return (1 if misc_gain >= -5.0 and flange_gain >= -5.0 and narrow_gain >= -5.0 and maze_gain >= -5.0 else 0, misc_gain, mean_gain, flange_gain + narrow_gain, -mean_overhead)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx35:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx35:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx35:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx35_haa_cache')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx35_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx35_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx35:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_delta_parent = _delta(public_summary, ('dataset',), 'CX34-A (Full)')
        public_family_cx3 = _family_delta(public_rows, ('dataset',), 'CX3-D')
        public_family_parent = _family_delta(public_rows, ('dataset',), 'CX34-A (Full)')
        ...
        summary_rows.append(
            {
                ...
                'exp4_exp_delta_vs_cx3': float(full_cx3['exp_delta']),
                'exp4_overhead_vs_cx3': float(full_cx3['mean_time_overhead_ratio']),
                'exp4_exp_delta_vs_parent': float(full_parent['exp_delta']),
                'misc_delta_vs_parent': float(misc['exp_delta']),
                'maze_delta_vs_parent': float(maze['exp_delta']),
                'flange_delta_vs_parent': float(flange['exp_delta']),
                'narrow_delta_vs_parent': float(narrow['exp_delta']),
                'report_path': str(report_path),
                'out_root': str(out_root),
            }
        )

    summary_root = args.outputs_root / 'rs_p0cx35_round1_summary'
    ...
    for row in summary_rows:
        lines.append(
            f"- `{row['variant']}`: exp4 vs `CX3-D` exp_delta=`{row['exp4_exp_delta_vs_cx3']:.3f}`, "
            f"overhead=`{row['exp4_overhead_vs_cx3']:.6f}`, "
            f"vs parent exp_delta=`{row['exp4_exp_delta_vs_parent']:.3f}`, "
            f"misc=`{row['misc_delta_vs_parent']:.3f}`, maze=`{row['maze_delta_vs_parent']:.3f}`, "
            f"flange=`{row['flange_delta_vs_parent']:.3f}`, narrow=`{row['narrow_delta_vs_parent']:.3f}`"
        )
    ranked = sorted(summary_rows, key=lambda r: (r['exp4_exp_delta_vs_parent'], r['misc_delta_vs_parent'], r['flange_delta_vs_parent'] + r['narrow_delta_vs_parent']), reverse=True)
    lines += ['', '## Ordering']
    for idx, row in enumerate(ranked, start=1):
        lines.append(f"- rank {idx}: `{row['variant']}`")
    (args.reports_root / 'rs_p0cx35_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX35 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx35_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

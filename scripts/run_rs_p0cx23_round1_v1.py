...
from rs_cx23.common import build_frozen_shadow_teacher
...
MODULES = {
    'CX23-A': 'rs_cx23.cx23_a_sad',
    'CX23-B': 'rs_cx23.cx23_b_cad',
    'CX23-C': 'rs_cx23.cx23_c_haa',
    'CX23-D': 'rs_cx23.cx23_d_cce',
}
SLUG = {'CX23-A': 'a', 'CX23-B': 'b', 'CX23-C': 'c', 'CX23-D': 'd'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX23 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX23-A,CX23-B,CX23-C,CX23-D')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx23:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append({
            'dataset': dataset,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], val_family_rows: list[dict[str, Any]], public_delta: list[dict[str, Any]], public_family: list[dict[str, Any]], standard_rows: list[dict[str, Any]]) -> None:
    lines = [
        f'# {title}',
        '',
    ...
    lines += ['', '## Hard Benchmark vs `CX3-D`', '- skipped: this round intentionally stayed public-first / ordinary-support; no hard-test evidence was consumed.', '', '## Hard Family Breakdown', '- skipped: this round intentionally stayed public-first / ordinary-support; no hard-test evidence was consumed.', '', '## Standard Support Audit']
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx23:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx23:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx23:public')

    shadow_teacher = build_frozen_shadow_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx23_shadow_cache')
    dependencies = {'shadow_teacher': shadow_teacher}
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx23_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx23_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx23:{variant}] trial={idx} params={params_obj} summary={val_summary}', flush=True)
        ...
        _variant_report(report_path, f'{variant} Pilot V1', chosen_json, val_family_rows, public_delta, public_family, standard_rows)
    ...
    summary_root = args.outputs_root / 'rs_p0cx23_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx23_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX23 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx23_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

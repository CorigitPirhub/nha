...
from rs_cx24.common import build_frozen_haa_teacher, observatory_summary
...
MODULES = {
    'CX24-E': 'rs_cx24.cx24_e_ato',
    'CX24-A': 'rs_cx24.cx24_a_mtw',
    'CX24-D': 'rs_cx24.cx24_d_ccc',
    'CX24-B': 'rs_cx24.cx24_b_tas',
    'CX24-C': 'rs_cx24.cx24_c_gra',
}
SLUG = {'CX24-E': 'e', 'CX24-A': 'a', 'CX24-D': 'd', 'CX24-B': 'b', 'CX24-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX24 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX24-E,CX24-A,CX24-D,CX24-B,CX24-C')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx24:standard:{dataset}] {idx}/{len(files)}', flush=True)
        rows.append({
            'dataset': dataset,
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx24:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx24:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx24:public')

    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx24_haa_cache')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx24_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx24_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx24:{variant}] trial={idx} params={params_obj} summary={val_summary}', flush=True)
        ...
        if diagnostic_rows:
            diag_summary = observatory_summary([
                {
                    'scenario': row['scenario'],
                    'sample_name': row.get('sample_name', ''),
                    'auto_state': row.get('auto_state', 'observe'),
                    'class_key': row.get('class_key', 'uncertain|none'),
                    'future_gain': 0.0,
                    'support_count': row.get('support_count', 0),
                    'recover_left': row.get('recover_left', 0),
                    'macro_count': row.get('macro_count', 0),
                }
                for row in diagnostic_rows
            ])
            observatory_note = f"diagnostic rows saved to `outputs/rs_p0cx24_{slug}_pilot_v1/diagnostic_rows.csv`; state_counts=`{diag_summary['state_counts']}`"
    ...
    summary_root = args.outputs_root / 'rs_p0cx24_round1_summary'
    ...
    (args.reports_root / 'rs_p0cx24_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX24 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx24_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

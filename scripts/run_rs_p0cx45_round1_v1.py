...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX45 round1 quality-calibrated witness transfer pilots.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx45_a_pilot_v1'))
...
def _eval_variant(mod, memory, params_obj, predictor, cfg, assets, cap, device, method_name, *, ablation=None):
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        rows.append({'dataset': 'exp4', 'sample_name': str(asset['path'].name), 'scenario': str(asset['case']['scenario']), 'method': method_name, 'success': float(plan.success), 'expansions': float(plan.expansions), 'time_ms': float(plan.runtime_ms + prep_ms), 'witness_hits': float(stats.get('witness_hits', 0.0)), 'qc_skip_hits': float(stats.get('qc_skip_hits', 0.0)), 'qc_conf_mean': float(stats.get('qc_conf_sum', 0.0) / max(stats.get('qc_conf_count', 1.0), 1e-6)), 'family_gate_hits': float(stats.get('family_gate_hits', 0.0)), 'family_gate_bypass': float(stats.get('family_gate_bypass', 0.0))})
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx45:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main() -> None:
    ...
    mod = importlib.import_module('rs_cx45.cx45_a_qcwt')
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx45:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx45:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx45:public')
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        ...
        trial_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX45-A (Candidate)')
        ...
        print(f'[cx45] trial={idx} score={score:.3f} success_gain={success_gain:.6f} exp_gain={exp_gain:.3f} time_gain={time_gain:.3f} params={params_obj}', flush=True)
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX45-A (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, f"CX45-A ({spec['name']})", ablation=spec))
    ...
    (args.outputs_root / 'chosen.json').write_text(json.dumps({'variant': 'CX45-A', 'params': params_obj.__dict__, 'selection_score': float(chosen['score']), 'val_success_gain': float(chosen['success_gain']), 'val_exp_gain': float(chosen['exp_gain']), 'val_time_gain': float(chosen['time_gain'])}, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    (args.reports_root / 'rs_p0cx45_a_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

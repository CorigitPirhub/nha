...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX43 round3 inline structural rank release pilots.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx43_c_pilot_v1'))
...
def _eval_variant(mod, memory, params_obj, predictor, cfg, assets, cap, device, method_name, *, ablation=None):
    ...
    for idx, asset in enumerate(assets, start=1):
        ...
        rows.append({'dataset': 'exp4', 'sample_name': str(asset['path'].name), 'scenario': str(asset['case']['scenario']), 'method': method_name, 'success': float(plan.success), 'expansions': float(plan.expansions), 'time_ms': float(plan.runtime_ms + prep_ms)})
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx43c:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main() -> None:
    ...
    print('[cx43c] boot', flush=True)

    mod = importlib.import_module('rs_cx43.cx43_c_isrr')
    ...
    print(f'[cx43c] load assets train={len(train_files)} val={len(val_files)} public={len(public_files)}', flush=True)
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx43c:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx43c:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx43c:public')
    ...
    parent_memory = parent_mod.fit_variant(train_assets, val_contexts, predictor, cfg, parent_params, args.outputs_root / 'parent_fit', args.device, {'haa_teacher': teacher})
    compat_memory = compat_mod.fit_variant(train_assets, val_contexts, predictor, cfg, compat_params, args.outputs_root / 'compat_fit', args.device, {'haa_teacher': teacher})

    print('[cx43c] eval parent on val', flush=True)
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        print(f'[cx43c] trial {idx} fit/eval', flush=True)
        fit_dir = args.outputs_root / 'trials' / f'trial_{idx:02d}'
        memory = mod.fit_variant(train_assets, val_contexts, predictor, cfg, params_obj, fit_dir, args.device, {'haa_teacher': teacher})
        trial_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX43-C (Candidate)')
        ...
        print(f'[cx43c] trial={idx} score={score:.3f} success_gain={success_gain:.6f} exp_gain={exp_gain:.3f} time_gain={time_gain:.3f} params={params_obj}', flush=True)
    chosen = sorted(trials, key=lambda item: item['score'], reverse=True)[0]
    params_obj = chosen['params_obj']
    memory = chosen['memory']

    print('[cx43c] unified public eval', flush=True)
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX43-C (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, f"CX43-C ({spec['name']})", ablation=spec))
    ...
    print('[cx43c] standard audit', flush=True)
    ...
    (args.outputs_root / 'chosen.json').write_text(json.dumps({'variant': 'CX43-C', 'params': params_obj.__dict__, 'selection_score': float(chosen['score']), 'val_success_gain': float(chosen['success_gain']), 'val_exp_gain': float(chosen['exp_gain']), 'val_time_gain': float(chosen['time_gain'])}, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    (args.reports_root / 'rs_p0cx43_c_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

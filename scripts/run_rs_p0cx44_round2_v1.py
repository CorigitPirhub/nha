...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX44 round2 family-conditional witness transfer pilots.')
    ...
    p.add_argument('--outputs-root', type=Path, default=Path('outputs/rs_p0cx44_b_pilot_v1'))
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx44b:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main() -> None:
    ...
    mod = importlib.import_module('rs_cx44.cx44_b_fcwt')
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx44b:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx44b:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx44b:public')
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        ...
        trial_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, 'CX44-B (Candidate)')
        ...
        print(f'[cx44b] trial={idx} score={score:.3f} success_gain={success_gain:.6f} exp_gain={exp_gain:.3f} time_gain={time_gain:.3f} params={params_obj}', flush=True)
    ...
    rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, 'CX44-B (Full)'))
    for spec in mod.ablation_specs():
        rows.extend(_eval_variant(mod, memory, params_obj, predictor, cfg, public_contexts, int(args.exp4_cap), args.device, f"CX44-B ({spec['name']})", ablation=spec))
    ...
    (args.outputs_root / 'chosen.json').write_text(json.dumps({'variant': 'CX44-B', 'params': params_obj.__dict__, 'selection_score': float(chosen['score']), 'val_success_gain': float(chosen['success_gain']), 'val_exp_gain': float(chosen['exp_gain']), 'val_time_gain': float(chosen['time_gain'])}, indent=2, ensure_ascii=False), encoding='utf-8')
    ...
    (args.reports_root / 'rs_p0cx44_b_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

...
from rs_cx8.common import FitArtifact, load_fit_model, load_nonholonomic_assets, run_hybrid_with_policy, sha256_file, write_inputs_sha256


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run optimized CX8-B hard-family pilot on calib_hard_v1 only.')
    ...
    p.add_argument('--out-root', type=Path, default=Path('outputs/rs_p0cx8_optimized_hard_pilot_v1'))
...
def _choose_trial(trials: list[dict[str, Any]]) -> dict[str, Any]:
    feasible = []
    for tr in trials:
        if float(tr['val_summary']['time_overhead_ratio']) <= 0.10:
            feasible.append(tr)
    ranked = feasible if feasible else trials
    ranked.sort(key=lambda t: (float(t['val_summary']['success_delta_pp']), float(t['val_summary']['exp_delta']), -float(t['val_summary']['time_overhead_ratio'])), reverse=True)
    return ranked[0]
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.fixed_cap), tag='rs-p0cx8-hardpilot:train')
    val_assets = load_nonholonomic_assets(val_files, predictor, cfg, int(args.fixed_cap), tag='rs-p0cx8-hardpilot:val')

    mod = importlib.import_module('rs_cx8.cx8_b_kfm')
    trials = []
    ...
    for idx, params_obj in enumerate(mod.param_grid(), start=1):
        ...
        val_summary = {
            ...
            'path_delta': float(np.nanmean([float(r['baseline_path_length']) - float(r['cx_path_length']) for r in case_rows])) if case_rows else float('nan'),
        }
        trials.append({'params_obj': params_obj, 'memory': memory, 'fit_dir': fit_dir, 'case_rows': case_rows, 'val_summary': val_summary})
        print(f'[rs-p0cx8-hardpilot] trial={idx} params={params_obj} summary={val_summary}')
    ...
    manifest = {
        'version': 'rs_p0cx8_optimized_hard_pilot_v1',
    ...
    if float(chosen_json['val_summary']['exp_delta']) > 0.0 and float(chosen_json['val_summary']['time_overhead_ratio']) <= 0.10:
        ...
        lines.append('- result: no positive cross-family pilot trend yet under the optimized implementation')
    (args.reports_root / 'rs_p0cx8_optimized_hard_pilot_v1.md').write_text('\n'.join(lines), encoding='utf-8')

...
MODULES = {'CX40-B': 'rs_cx40.cx40_b_mwc'}
SLUG = {'CX40-B': 'b'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX40 round2 pilots.')
    p.add_argument('--variants', type=str, default='CX40-B')
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx40b:standard:{dataset}] {idx}/{len(files)}', flush=True)
...
def main():
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx40b:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx40b:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx40b:public')
    haa_teacher = build_frozen_haa_teacher(train_assets, val_contexts, predictor, cfg, args.device, args.outputs_root / 'rs_p0cx40b_haa_cache')
    ...
    parent_memory = parent_mod.fit_variant(train_assets, val_contexts, predictor, cfg, parent_params, args.outputs_root / 'parent_fit_cx40b', args.device, {'haa_teacher': haa_teacher})
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx40_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx40_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid(), start=1):
            ...
            print(f'[cx40b:{variant}] trial={idx} params={params_obj} score={score}', flush=True)
        ...
        public_rows.extend(cx36_public_rows)
        public_rows.extend(cx39c_public_rows)
        public_rows.extend(cx40a_public_rows)
        ...
        public_delta_cx39c = _delta(public_summary, ('dataset',), 'CX39-C (Full)')
        public_delta_cx36 = _delta(public_summary, ('dataset',), 'CX36-B (Full)')
        ...
        _write_csv(out_root / 'public_delta_vs_cx39c.csv', public_delta_cx39c)
        _write_csv(out_root / 'public_delta_vs_cx36.csv', public_delta_cx36)
        ...
        lines += ['', '## Public vs `CX39-C (Full)`']
        for row in public_delta_cx39c:
            lines.append(f"- `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
    ...
    summary_root = args.outputs_root / 'rs_p0cx40_round2_summary'
    ...
    lines = ['# P0-CX40 Round2 Summary', '', '- protocol: frozen `CX40-A` parent on public evidence; dev-only selection on `calib_hard_v1`; locked public `parasol_narrow exp4` evaluation + `mp/csm` ordinary-support audit', '', '## Variant Readout']
    for row in summary_rows:
        lines.append(f"- `{row['variant']}`: vs `CX40-A` exp_delta=`{row['exp4_exp_delta_vs_cx40a']:.3f}`, runtime_ratio=`{row['exp4_runtime_vs_cx40a']:.6f}`, misc=`{row['misc_delta_vs_cx40a']:.3f}`, maze=`{row['maze_delta_vs_cx40a']:.3f}`, flange=`{row['flange_delta_vs_cx40a']:.3f}`, narrow=`{row['narrow_delta_vs_cx40a']:.3f}`")
    (args.reports_root / 'rs_p0cx40_round2_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX40 Round2 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx40_round2_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

...
MODULES = {
    'CX21-A': 'rs_cx21.cx21_a_cvf',
    'CX21-B': 'rs_cx21.cx21_b_lag',
    'CX21-C': 'rs_cx21.cx21_c_scg',
}
SLUG = {'CX21-A': 'a', 'CX21-B': 'b', 'CX21-C': 'c'}


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Run P0-CX21 round1 pilots.')
    p.add_argument('--variants', type=str, default='CX21-A,CX21-B,CX21-C')
    p.add_argument('--trial-limit', type=int, default=1)
    p.add_argument('--public-datasets', type=str, default='exp4')
    p.add_argument('--ablation-limit', type=int, default=1)
    ...
    for dataset, limit in [('mp', int(max_mp_cases)), ('csm', int(max_csm_cases))]:
        ...
        for idx, path in enumerate(files, start=1):
            ...
            if idx % 100 == 0 or idx == len(files):
                print(f'[cx21:standard:{dataset}] {idx}/{len(files)}')
        rows.append({
            'dataset': dataset,
...
def _variant_report(path: Path, title: str, chosen_json: dict[str, Any], val_family_rows: list[dict[str, Any]], public_delta: list[dict[str, Any]], public_family: list[dict[str, Any]], hard_delta: list[dict[str, Any]], hard_family: list[dict[str, Any]], standard_rows: list[dict[str, Any]], *, hard_skipped_note: str | None = None) -> None:
    lines = [
        f'# {title}',
        '',
        '- protocol: dev-only selection on `calib_hard_v1`; locked public `parasol_narrow` evaluation; hard-test is consumed only when public overall turns positive and `flange` stays non-negative',
    ...
    if hard_delta:
        ...
        lines.append(f"- {hard_skipped_note}" if hard_skipped_note else '- skipped: public positive ceiling gate did not clear, so hard-test evidence was not consumed.')
    lines += ['', '## Hard Family Breakdown']
    if hard_family:
        for row in hard_family:
            if row['method'].endswith('(Full)'):
                lines.append(f"- `{row['scenario']}` / `{row['method']}`: success_delta_pp=`{float(row['success_delta_pp']):.3f}`, exp_delta=`{float(row['exp_delta']):.3f}`, mean_time_overhead_ratio=`{float(row['mean_time_overhead_ratio']):.6f}`")
    else:
        lines.append(f"- {hard_skipped_note}" if hard_skipped_note else '- skipped: no hard-family rows because hard escalation was not triggered.')
    ...
    lines += ['', '## Final Readout']
    full = next((r for r in public_delta if r['dataset'] == 'exp4' and r['method'].endswith('(Full)')), None)
    flange = next((r for r in public_family if r['dataset'] == 'exp4' and r['method'].endswith('(Full)') and r['scenario'] == 'flange'), None)
    if full is None or flange is None:
        lines.append('- result: missing public rows.')
    else:
        passed = float(full['exp_delta']) > 0.0 and float(flange['exp_delta']) >= 0.0
        lines.append('- result: public ceiling gate cleared.' if passed else '- result: public ceiling gate not cleared under the locked protocol.')
...
def main() -> None:
    ...
    train_assets = load_nonholonomic_assets(train_files, predictor, cfg, int(args.dev_cap), tag='cx21:calib-train')
    val_contexts = load_nonholonomic_contexts(val_files, predictor, cfg, tag='cx21:calib-val')
    public_contexts = load_nonholonomic_contexts(public_files, predictor, cfg, tag='cx21:public')
    ...
    for variant in variants:
        ...
        slug = SLUG[variant]
        out_root = args.outputs_root / f'rs_p0cx21_{slug}_pilot_v1'
        out_root.mkdir(parents=True, exist_ok=True)
        report_path = args.reports_root / f'rs_p0cx21_{slug}_pilot_v1.md'
        trials = []
        for idx, params_obj in enumerate(mod.param_grid()[: int(max(args.trial_limit, 1))], start=1):
            fit_dir = out_root / 'trials' / f'trial_{idx:02d}'
            memory = mod.fit_variant(train_assets, val_contexts, predictor, cfg, params_obj, fit_dir, args.device)
            val_rows = _eval_variant(mod, memory, params_obj, predictor, cfg, val_contexts, int(args.dev_cap), args.device, f'{variant} (Candidate)')
            val_case_rows = []
            ...
            print(f'[cx21:{variant}] trial={idx} params={params_obj} summary={val_summary}')
        ...
        for dataset in public_datasets:
            ...
            variant_rows = _eval_variant(mod, chosen_memory, chosen_params, predictor, cfg, public_contexts, int(cap), args.device, f'{variant} (Full)')
            for row in variant_rows:
                row['dataset'] = dataset
            public_rows.extend(variant_rows)
            if dataset == 'exp4':
                for spec in getattr(mod, 'ablation_specs', lambda: [])()[: int(max(args.ablation_limit, 0))]:
        ...
        should_run_hard = bool((not bool(args.skip_hard)) and full_exp4 is not None and flange is not None and float(full_exp4['exp_delta']) > 0.0 and float(flange['exp_delta']) >= 0.0)

        hard_delta: list[dict[str, Any]] = []
        hard_family: list[dict[str, Any]] = []
        if should_run_hard:
            hard_contexts = load_nonholonomic_contexts(hard_files, predictor, cfg, tag=f'cx21:{variant}:hard-test')
            hard_rows = []
            ...
            if not hard_rows:
                raise RuntimeError('Hard baseline cache missing; refusing to consume hard test without locked baseline reference.')
            hard_variant_rows = _eval_variant(mod, chosen_memory, chosen_params, predictor, cfg, hard_contexts, int(args.hard_cap), args.device, f'{variant} (Full)')
            for row in hard_variant_rows:
                row['dataset'] = 'hard_test'
            hard_rows.extend(hard_variant_rows)
            _write_csv(out_root / 'hard_case_rows.csv', hard_rows)
            hard_summary = _summary(hard_rows, ('method',))
            hard_delta = _delta(hard_summary, tuple(), baseline_method='CX3-D')
            hard_family = _family_delta(hard_rows, tuple(), baseline_method='CX3-D')
            _write_csv(out_root / 'hard_summary.csv', hard_summary)
            _write_csv(out_root / 'hard_delta.csv', hard_delta)
            _write_csv(out_root / 'hard_family_delta.csv', hard_family)
        ...
        hard_skipped_note = 'skipped: this round intentionally stayed public-first / ordinary-support (`--skip-hard`), so no hard-test evidence was consumed.' if bool(args.skip_hard) else None
        _variant_report(report_path, f'{variant} Pilot V1', chosen_json, val_family_rows, public_delta, public_family, hard_delta, hard_family, standard_rows, hard_skipped_note=hard_skipped_note)
        ...
        summary_rows.append({
            ...
            'hard_escalated': bool(should_run_hard),
            'report_path': str(report_path),
            'out_root': str(out_root),
        })

    summary_root = args.outputs_root / 'rs_p0cx21_round1_summary'
    ...
    lines = [
        ...
        '',
        '- protocol: dev-only selection on `calib_hard_v1`; locked public `parasol_narrow` evaluation; this round ran in public-first / ordinary-support mode and did not consume hard-test evidence',
        '',
        '## Variant Readout',
    ]
    for row in summary_rows:
        lines.append(f"- `{row['variant']}`: calib_val exp_delta=`{row['calib_val_exp_delta']:.3f}`, calib_val overhead=`{row['calib_val_overhead']:.6f}`, exp4 exp_delta=`{row['exp4_exp_delta']:.3f}`, exp4 overhead=`{row['exp4_overhead']:.6f}`, flange exp_delta=`{row['flange_exp_delta']:.3f}`, hard_escalated=`{int(bool(row['hard_escalated']))}`")
    ...
    (args.reports_root / 'rs_p0cx21_round1_summary.md').write_text('\n'.join(lines), encoding='utf-8')

    audit_lines = ['# P0-CX21 Standard Audit V1', '', '- protocol: ordinary-support audit checks `build_standard_field == accepted CX3-D` by construction', '']
    ...
    (args.reports_root / 'rs_p0cx21_standard_audit_v1.md').write_text('\n'.join(audit_lines), encoding='utf-8')

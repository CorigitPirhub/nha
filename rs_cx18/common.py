...
from rs_cx17.common import FEATURE_NAMES, MotifEdge, feature_vector
...
def state_feature_vector(stats, margin: float, oracle_gain: float, spec: RecoverabilitySpec) -> np.ndarray:
    return feature_vector(stats, margin, oracle_gain, spec)
    ...
    *,
    ...
    for asset in train_assets:
        ...
        for idx in range(min(len(trace) - 1, len(margins) - 2)):
            ...
            feat = state_feature_vector(stats, margins[idx], oracle_gain, spec)
            rows_by_state.setdefault(state, {}).setdefault(str(macro.name), []).append(feat)
            gains_by_state.setdefault(state, {}).setdefault(str(macro.name), []).append(gain)
            counts_by_state.setdefault(state, {})[str(macro.name)] = int(counts_by_state[state].get(str(macro.name), 0)) + 1
...
def choose_state_macros(
    ...
    *,
    ...
    for macro in macros:
        ...
        if matched:
            score = float(sim) + 0.03 * float(macro.avg_gain) + 0.01 * float(count_state.get(str(macro.name), 0.0))
...
def compile_motif_compiler_graph(
    ...
    *,
    horizon_steps: int,
    min_gain: float,
) -> tuple[dict[str, list[MotifEdge]], dict[str, dict[str, SupportBand]]]:
    ...
    for asset in train_assets:
        ...
        for idx in range(min(len(trace) - 2, len(margins) - 3)):
            ...
            feat = state_feature_vector(entry_stats, margins[idx], entry_oracle_gain, spec)

...
- inputs sha256: `outputs/rs_p0cx8_d_lite_reorder_pilot_v1/inputs_sha256.json`
...
## Final Verdict
- `CX8-D-Lite-Reorder` fails the gate for promotion.
- It reduces runtime overhead to `0.1630`, but this still exceeds the target `<0.10`; meanwhile `exp_delta` collapses to `0.0`.
- Therefore the positive gain of heavy `CX8-D` does not survive the lightweight rewrite, and the `P0-CX8` route should be frozen as failed under the current compute constraint.

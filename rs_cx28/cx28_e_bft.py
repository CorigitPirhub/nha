...
def param_grid() -> list[CX28EBFTParams]:
    return [
        CX28EBFTParams(2, 18, 0.10, 0.54, 1, 0.15, 0.06, 0.10, 0.01, 32, 0.02, 0.05, 32, 16, 2, 24),
        CX28EBFTParams(2, 18, 0.10, 0.54, 1, 0.15, 0.06, 0.09, 0.01, 32, 0.02, 0.05, 32, 16, 2, 24),
        CX28EBFTParams(2, 14, 0.08, 0.50, 1, 0.10, 0.05, 0.08, 0.00, 28, 0.02, 0.04, 28, 16, 2, 24),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Bridge-Filtered-Turn', 'disable_bridge_filtered_turn': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX28EBFTParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'bft_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class BFTPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX28EBFTParams, memory: dict[str, Any], disable_bridge_filtered_turn: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_bridge_filtered_turn = bool(disable_bridge_filtered_turn)
    ...
    def _misc_shortlist(self) -> list[str]:
        ...
        if barrier_peak >= 0.8 and bridge_diffuse <= float(self.params.bridge_thr):
            return ['forward_safe|straight', 'forward_safe|forward_turn', 'uncertain|none']
        return ['forward_safe|straight', 'uncertain|none']
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if (not self.disable_bridge_filtered_turn) and str(self.scene_kind) == 'misc' and str(search_state.get('haa_state', 'observe')) in {'candidate', 'commit'}:
                ...
                if scored:
                    ...
                    if best_key != current_key and best_score >= current_score + float(self.params.switch_margin):
                        if best_key == 'uncertain|none':
                            ...
                            guard_reason = 'misc_bridge_abstain'
                        else:
                            ...
                            guard_reason = f'misc_bridge:{best_key}'
...
def make_policy(memory: dict[str, Any], params: CX28EBFTParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return BFTPolicy(
        ...
        params,
        memory,
        disable_bridge_filtered_turn=bool(ablation.get('disable_bridge_filtered_turn', False)),
        enable_diagnostics=bool(ablation.get('enable_diagnostics', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX28EBFTParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX28EBFTParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

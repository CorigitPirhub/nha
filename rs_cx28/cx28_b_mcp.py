...
def param_grid() -> list[CX28BMCPParams]:
    return [
        CX28BMCPParams(2, 18, 0.10, 0.54, 1, 0.20, 0.02, 0.00, 48, 0.02, 0.05, 32, 16, 2, 24),
        CX28BMCPParams(2, 18, 0.10, 0.54, 1, 0.15, 0.01, -0.01, 40, 0.02, 0.05, 32, 16, 2, 24),
        CX28BMCPParams(2, 14, 0.08, 0.50, 1, 0.10, 0.00, -0.02, 32, 0.02, 0.04, 28, 16, 2, 24),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Class-Precondition', 'disable_misc_precondition': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX28BMCPParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'mcp_meta.json').write_text(json.dumps({'params': params.__dict__}, indent=2, ensure_ascii=False), encoding='utf-8')
    return {'haa_teacher': haa_teacher, 'best_val_loss': float('nan')}


class MCPPPolicy(BaseCX28Policy):
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, params: CX28BMCPParams, memory: dict[str, Any], disable_misc_precondition: bool = False, enable_diagnostics: bool = False) -> None:
        self.params = params
        self.disable_misc_precondition = bool(disable_misc_precondition)
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        if (not self.disable_misc_precondition) and str(self.scene_kind) == 'misc' and str(search_state.get('haa_state', 'observe')) in {'candidate', 'commit'}:
                ...
                if trigger and planner is not None and h_pair is not None:
                    ...
                    if scored:
                        ...
                        if best_key != current_key and best_score >= current_score + float(self.params.switch_margin):
                            if best_key == 'uncertain|none' or best_score <= primitive_score + float(self.params.abstain_margin):
                                ...
                                guard_reason = 'misc_precondition_abstain'
                            else:
                                ...
                                guard_reason = f'misc_precondition:{best_key}'
...
def make_policy(memory: dict[str, Any], params: CX28BMCPParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ...
    return MCPPPolicy(
        ...
        params,
        memory,
        disable_misc_precondition=bool(ablation.get('disable_misc_precondition', False)),
        enable_diagnostics=bool(ablation.get('enable_diagnostics', False)),
    )


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX28BMCPParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX28BMCPParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    from rs_cx23 import cx23_c_haa as base_mod

...
def param_grid() -> list[CX44DSWRTParams]:
    common = dict(
    ...
    return [
        CX44DSWRTParams(**common, base_ttl=32, ttl_bonus=64, confidence_floor=0.70, anchor_scale=1.5),
        CX44DSWRTParams(**common, base_ttl=24, ttl_bonus=48, confidence_floor=0.65, anchor_scale=1.8),
        CX44DSWRTParams(**common, base_ttl=40, ttl_bonus=72, confidence_floor=0.75, anchor_scale=1.2),
    ]
...
def _family_enabled(params: CX44DSWRTParams, scenario: str) -> bool:
    return bool(
...
def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX44DSWRTParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    (out_dir / 'cx44_d_meta.json').write_text(
        json.dumps(
            {
...
class SWRTPolicy(parent_mod.FCWTPolicy):
    def __init__(
        self,
        case: dict[str, Any],
        bundle: dict[str, Any],
        ...
        *,
        ...
        self.stats['soft_weight_sum'] = 0.0
        self.stats['soft_weight_count'] = 0.0
        self.stats['soft_skip_hits'] = 0.0

    def start_search(self, planner, start, goal, h_pair, search_state):
        search_state.setdefault('cx44d_soft_witness', {})
        search_state.setdefault('cx44d_pending_sig', None)
    ...
    def _weight(self, sig: tuple[Any, ...]) -> float:
        ...
        weight = float(1.0 / (1.0 + np.exp(-3.0 * z)))
        return float(np.clip(weight, 0.0, 1.0))

    def _lookup_soft_witness(self, sig: tuple[Any, ...], current_anchor: float, current_popped: int, search_state: dict[str, Any]) -> dict[str, Any] | None:
        witness = dict(search_state.get('cx44d_soft_witness', {})).get(sig)
        if not isinstance(witness, dict):
            return None
        if int(current_popped) > int(witness.get('expiry', -1)):
            return None
        weight = float(witness.get('weight', 0.0))
        anchor_allow = float(current_anchor) + float(self.soft_params.anchor_eps) * (1.0 + float(self.soft_params.anchor_scale) * weight) >= float(witness.get('best_anchor', current_anchor))
        confident = float(witness.get('support', 0.0)) >= float(self.soft_params.confidence_floor)
        if bool(anchor_allow and confident):
            return witness
        return None
    ...
    def extra_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ...
        weight = self._weight(sig)
        self.stats['soft_weight_sum'] = float(self.stats.get('soft_weight_sum', 0.0) + weight)
        self.stats['soft_weight_count'] = float(self.stats.get('soft_weight_count', 0.0) + 1.0)
        ...
        if self.disable_witness_transfer:
            search_state['cx44d_pending_sig'] = (sig, weight)
            self.stats['witness_full_reviews'] = float(self.stats.get('witness_full_reviews', 0.0) + 1.0)
            return self.base.extra_successors(planner, record, goal, records, candidates, node_ctx, search_state, h_pair)
        witness = self._lookup_soft_witness(sig, current_anchor, current_popped, search_state)
        if self.force_negative_skip and len(list(node_ctx.get('macros', []))) > 0:
            ...
            self.stats['soft_skip_hits'] = float(self.stats.get('soft_skip_hits', 0.0) + 1.0)
            search_state['cx44d_pending_sig'] = None
            return []
    ...
    def rank_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        ranked = self.base.rank_successors(planner, record, goal, records, candidates, node_ctx, search_state, h_pair)
        pending = search_state.get('cx44d_pending_sig')
        ...
        if (not top_is_macro) and macro_scores:
            ...
            excess = max(float(margin) - 0.5 * float(self.soft_params.margin_thr), 0.0)
            strength = float(weight * excess / max(float(self.soft_params.margin_thr), 1e-6))
            witness_map = dict(search_state.get('cx44d_soft_witness', {}))
            current_anchor = float(getattr(record, 'anchor', 0.0))
            current_popped = int(search_state.get('popped', 0))
            ...
            witness_map[sig] = {
                ...
                'support': float(prev_support + strength),
                'weight': float(weight),
                'expiry': int(max(int(prev.get('expiry', current_popped)) if isinstance(prev, dict) else current_popped, current_popped + int(self.soft_params.base_ttl + round(self.soft_params.ttl_bonus * weight)))),
            }
            search_state['cx44d_soft_witness'] = witness_map
            self.stats['witness_store_negative'] = float(self.stats.get('witness_store_negative', 0.0) + 1.0)
        search_state['cx44d_pending_sig'] = None
        return ranked


def make_policy(memory: dict[str, Any], params: CX44DSWRTParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
...
def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX44DSWRTParams, memory: dict[str, Any] | None = None) -> np.ndarray:
...
def build_standard_field(sample, predictor, params: CX44DSWRTParams, memory: dict[str, Any] | None = None) -> np.ndarray:

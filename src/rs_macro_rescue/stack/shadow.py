...
from rs_macro_rescue.stack.support import TreeNode, fit_tree, predict_tree, tree_to_dict
from rs_macro_rescue.stack.primitive import primitive_group
from rs_macro_rescue.stack import legality_policy as lag_mod
from rs_macro_rescue.stack.legality import family_bucket_name
from rs_macro_rescue.stack.legality import run_hybrid_with_policy
from rs_macro_rescue.stack.nonholonomic import query_yaw_field
...
FROZEN_TEACHER_PARAMS = lag_mod.CX21BLAGParams(
    support_slack=0.18,
    allowed_bonus=0.08,
    discouraged_penalty=0.06,
    forbidden_penalty=0.10,
    macro_bonus=0.08,
    must_precede_bonus=0.10,
    improve_gain=0.14,
    hard_forbid_hits=6,
    max_macros=3,
    forward_viability_thr=0.34,
    reverse_required_thr=0.08,
    trap_high_thr=0.56,
    escape_affinity_low_thr=-0.02,
    hopeless_viability_thr=0.10,
    stride_cells=2,
    yaw_stride=2,
    horizon_steps=5,
)
...
def load_frozen_teacher_params() -> lag_mod.CX21BLAGParams:
    return FROZEN_TEACHER_PARAMS

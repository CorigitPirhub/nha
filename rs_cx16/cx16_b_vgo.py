...
def param_grid() -> list[CX16BVGOParams]:
    return [
        CX16BVGOParams(0.04, 0.18, 0.04, 1, 0.18, 0.08, 0.06, 0.08, 0.22, 0.24, 0.30, 0.20, 0.10, 0.06, 0.08, 2, 2, 4),
        CX16BVGOParams(0.03, 0.16, 0.03, 1, 0.20, 0.10, 0.08, 0.10, 0.22, 0.24, 0.32, 0.22, 0.10, 0.06, 0.08, 2, 2, 4),
        CX16BVGOParams(0.02, 0.14, 0.02, 2, 0.22, 0.12, 0.10, 0.12, 0.20, 0.26, 0.34, 0.22, 0.10, 0.04, 0.08, 2, 2, 5),
    ]


def ablation_specs() -> list[dict[str, Any]]:
    return [{'name': 'No-Oracle', 'disable_oracle': True}]


def fit_variant(calib_train_assets, calib_val_assets, predictor, cfg: CXGlobalConfig, params: CX16BVGOParams, out_dir: Path, device: str, dependencies: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    serializable = {','.join(map(str, key)): value for key, value in table.items()}
    save_meta(out_dir / 'vgo_meta.json', {'params': params.__dict__, 'table': serializable})
    return {'viability_table': table, 'best_val_loss': float('nan')}


class VGOPolicy:
    def __init__(self, case: dict[str, Any], bundle: dict[str, Any], params: CX16BVGOParams, memory: dict[str, Any], disable_oracle: bool = False) -> None:
        self.case = case
        self.params = params
        self.disable_oracle = bool(disable_oracle)
        ...
        self.seen_slot = '_cx16_b_seen'
        self.stall_slot = '_cx16_b_stall'
    ...
    def prepare_expand(self, planner, record, goal, records, open_heap, anchor_heap, search_state, h_pair):
        ...
        oracle = None if self.disable_oracle else query_viability_table(self.table, key)
        seen = read_slot_counter(search_state, self.seen_slot, stats.key)
        active = bool(
            (not self.disable_oracle)
            and (
                float(margin) <= float(self.params.margin_gate)
                or (oracle is not None and float(oracle.get('avg_future_gain', 0.0)) >= float(self.params.oracle_gain_thr))
                or int(seen) >= int(self.params.repeat_trigger)
            )
        )
        return {'stats': stats, 'margin': float(margin), 'oracle': oracle, 'active': bool(active)}

    def rank_successors(self, planner, record, goal, records, candidates, node_ctx, search_state, h_pair):
        if self.disable_oracle or not isinstance(node_ctx, dict) or not bool(node_ctx.get('active', False)):
            return None
        ...
        oracle = node_ctx.get('oracle', None)
        ...
        for cand, stats in zip(candidates, cand_stats):
            ...
            delta -= float(self.params.table_gain_weight) * float(oracle_gain)
            if float(reverse_need) > 0.08 and int(cand.direction) < 0:
                delta -= float(self.params.reverse_bonus)
            if float(cand_margin) <= float(self.params.hopeless_margin):
    ...
    def complete_expand(self, planner, record, goal, records, node_ctx, invalid_local, valid_local, accepted_local, search_state, h_pair):
        ...
        event_hit = bool(int(accepted_local) == 0 or float(node_ctx.get('margin', 1.0)) <= float(self.params.margin_gate))
        update_global_stall(search_state, self.stall_slot, bool(event_hit))


def make_policy(memory: dict[str, Any], params: CX16BVGOParams, case: dict[str, Any], bundle: dict[str, Any], field: np.ndarray, device: str, ablation: dict[str, Any] | None = None):
    ablation = ablation if isinstance(ablation, dict) else {}
    return VGOPolicy(case, bundle, params, memory, disable_oracle=bool(ablation.get('disable_oracle', False)))


def build_nonholonomic_field(case: dict[str, Any], predictor, cfg: CXGlobalConfig, params: CX16BVGOParams, memory: dict[str, Any] | None = None) -> np.ndarray:
    bundle, field = build_base_field(case, predictor, cfg)
    case['_cx16_bundle'] = bundle
    return field.astype(np.float32)


def build_standard_field(sample, predictor, params: CX16BVGOParams, memory: dict[str, Any] | None = None) -> np.ndarray:
